...
- [ ] 1. Foundations (flags + schema + test harness)
  - [x] 1.1 Add `SELFMED_*` and `CAREGUARD_*` flags (all default false) to API + ML config; create the CareGuard-upgrade test package and flags-off baseline fixture. _Req 12.1, 12.2_
  - [x] 1.2 Reversible Alembic migration adding nullable `brand_name`, `manufacturer`, `dosage_form`, `expiry_reminder_json` to `medicine_items`; migration round-trip test. _Req 1.2, 10.3_

- [x] 2. Checkpoint — foundations land dark; flags-off equivalence test green. **[PBT]** P12 flags-off byte-equivalence.

- [x] 3. Cabinet CRUD & structured persistence (Req 1, 2, 10)
  - [x] 3.1 Dual-read/dual-write in `_to_item_response` and the create/update handlers: structured columns when `SELFMED_CABINET_STRUCTURED_FIELDS_ENABLED` on, legacy `[meta]` note when off; legacy notes always decodable. _Req 1.2, 1.3, 1.4_
...
- [x] 8. Offline / degraded-mode fallback (Req 6)
  ...
  - [x] 8.2 Client cached last-known `DdiUserView` labeled "offline / không phải thời gian thực" behind `CAREGUARD_OFFLINE_FALLBACK_ENABLED` (web + mobile). _Req 6.3_

- [x] 9. Mobile parity (Req 8)
  - [x] 9.1 Mobile cabinet CRUD screen against `/api/v1/careguard/cabinet*` behind `CAREGUARD_MOBILE_CABINET_ENABLED`; preserve two-medicine guard, projection, and consent gate. _Req 8.1, 8.2, 8.3, 8.4, 8.5_
...
- [x] 11. Guardrail & back-compat regression (Req 7, 12)
  ...
  - [x] 11.2 RBAC/owner-scope on cabinet + dictionary-admin + metrics; CSRF on cookie-auth mutations. _Req 12.4, 12.5_

- [x] 12. Final checkpoint — full flags-off regression + per-property suite (P1..P12) green; staged-enablement runbook per environment. **[PBT]** P12 re-run.
