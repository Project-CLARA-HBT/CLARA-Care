...
- [ ] 1. Foundations (networking, flags, resilience, test harness)
  - [ ] 1.1 Add fakes (Api_Client, Session_Store, ConnectivityService) + recording analytics transport to `test/`; keep existing tests green.
  - [ ] 1.2 Add `connectivity_service.dart` (`isOnline` stream) and bounded request/stream timeouts in `api_client.dart` (Req 9.1, 9.2).
  - [ ] 1.3 Token refresh in `Api_Client`: pre-flight expiry check + single 401-retry via `POST /auth/refresh`; persist or clear session (Req 6.2, 6.3).
  - [ ] 1.4 **[PBT]** Token-refresh state machine: expired+valid-refresh ⇒ refresh; failed refresh ⇒ clear+login (Property P9).
  - [ ] 1.5 Mobile flag resolver: read `feature_flags` from `mobile/summary` + `--dart-define` defaults (all new flags default false) (Req 13.1, 15.1).

- [ ] 2. Checkpoint — foundations land dark; flags-off navigation equivalence test green (Property P1); existing tests still pass.

- [ ] 3. Chat parity (Req 1)
  - [ ] 3.1 `Api_Client.chat` + `streamChat` over `/chat` and `/chat/stream` (reuse SSE parser).
  - [ ] 3.2 `ChatScreen` with progressive streaming, citations, standing disclaimer, emergency fast-path; gated by `chat_mobile_enabled`.
  - [ ] 3.3 `EndUserSafeAnswer` widget — strip internal runtime fields for non-admin (Req 1.6).
...
- [ ] 4. Deep-research hardening (Req 2)
  ...
  - [ ] 4.2 Reaffirm role-gated rail (admin-only) + fail-closed block via `evaluateTelemetryGate`.
  - [ ] 4.3 **[Dart widget test]** Non-admin sees sanitized summary; unevaluable role blocks the job (Property P2).

- [ ] 5. Self-med cabinet + DDI parity (Req 3)
  - [ ] 5.1 `Api_Client` cabinet ops (`getCabinet`/`addCabinetItem`/`deleteCabinetItem`) over `/selfmed/*`.
...
- [ ] 7. Enhanced PHR parity (Req 5)
  - [ ] 7.1 Preserve legacy GET/PUT; reaffirm provenance/verification badges + persistent self-declared disclaimer (Req 5.1, 5.2, 5.3).
...
- [ ] 9. Transparency & model disclosure (Req 7)
  - [ ] 9.1 `model_disclosure.dart` (`fromResponse`, `isFallback` iff local synth) + `ModelDisclosureChip`; gated by `model_disclosure_mobile_enabled`.
  - [ ] 9.2 Versioned `AiTransparencyNotice` gate before medical content; gated by `transparency_notice_mobile_enabled`.
...
- [ ] 10. Consent center & DSAR (Req 8)
  - [ ] 10.1 `consent_state.dart` + `ConsentCenterScreen` purpose toggles; withdrawing analytics ⇒ `analytics.setConsent(false)` (Req 8.1, 8.2, 8.4).
...
- [ ] 11. Sharing & deep links (Req 12)
  - [ ] 11.1 `SharedResourceScreen` read-only by token via CLARA_API; invalid/expired ⇒ clear non-PII error; gated by `sharing_mobile_enabled`.
...
- [ ] 12. Offline, accessibility & error discipline (Req 9, 10, 11)
  - [ ] 12.1 `ErrorRetryView` / `OfflineBanner` on every data surface; block mutations offline with preserved input (Req 9.1, 9.4, 9.5).
  - [ ] 12.2 **[Dart widget test]** Offline shows error state + blocks mutations (Property P12).
  - [ ] 12.3 `a11y.dart`: semantics labels, ≥48dp targets, dynamic text scaling, reduced-motion resolver; status by text not color (Req 10.1, 10.2, 10.3, 10.4, 10.5).
  - [ ] 12.4 **[Dart widget test]** Interactive controls expose semantics + meet target; reduced-motion suppresses animation (Property P14).
  - [ ] 12.5 Screen-level exception containment (no crash, no stack trace to user) (Req 11.4).

- [ ] 13. Role-aware navigation (Req 13)
  - [ ] 13.1 Derive tiles from `mobile/summary` feature_flags; fail closed when summary unloadable; never expose admin-only surfaces to non-admin (Req 13.2, 13.4, 13.5).
