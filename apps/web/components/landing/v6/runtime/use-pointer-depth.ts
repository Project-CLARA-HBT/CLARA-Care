...
export interface PointerDepthOptions {
  maxRotateX?: number; // default 1.8deg
  maxRotateY?: number; // default 1.8deg
  maxTranslate?: number; // default 4px
  disabled?: boolean;
}
...
export function usePointerDepth<T extends HTMLElement = HTMLElement>(
  ...
  useEffect(() => {
    ...
    // Disabled on non-enhanced tiers or touch devices
    ...
    const maxRotX = Math.min(optionsRef.current.maxRotateX ?? 1.8, 2.5);
    const maxRotY = Math.min(optionsRef.current.maxRotateY ?? 1.8, 2.5);
    const maxTrans = Math.min(optionsRef.current.maxTranslate ?? 4, 6);
