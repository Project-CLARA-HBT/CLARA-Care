  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (selectedTier === "tier1") {
        ...
        let latestProgress = normalizeResearchTier2JobProgress({});
        const { finalPayload } = await executeResearchTier2Job(message, {
          ...
          onSnapshot: (snapshot) => {
            latestProgress = normalizeResearchTier2JobProgress(snapshot.progress);
            setLiveFlowStages(latestProgress.flowStages);
            setLiveFlowEvents(latestProgress.flowEvents);
            setLiveReasoningNotes(latestProgress.reasoningNotes);
            setLiveStatusNote(latestProgress.statusNote ?? "");
          },
          onStreamingFallback: (streamMessage) => {
            setLiveStatusNote(
              `${streamMessage} Đang tự động chuyển sang chế độ polling để giữ tiến trình.`
            );
          },
        });
        ...
        if (latestProgress.flowStages.length && !normalized.flowStages.length) {
          normalized.flowStages = latestProgress.flowStages;
        }
        if (latestProgress.flowEvents.length && !normalized.flowEvents.length) {
          normalized.flowEvents = latestProgress.flowEvents;
        }
        nextResult = {
          tier: "tier2",
          ...normalized
