    ...
    <div className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)_330px]">
      <aside className="space-y-4">
        <SurfaceCard className="overflow-hidden"><div className="border-b border-[color:var(--shell-border)] px-4 py-3"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">{copy("visits.listEyebrow")}</p><h2 className="mt-1 font-semibold text-[var(--text-primary)]">{copy("visits.choose")}</h2></div>
          {loading ? <div className="p-3"><LoadingCards count={2} /></div> : visits.length ? <div className="space-y-1 p-2">{visits.map((visit) => <button key={visit.id} type="button" onClick={() => choose(visit.id)} aria-pressed={selectedId === visit.id} className={"focus-ring w-full rounded-[var(--radius-lg)] p-3 text-left transition " + (selectedId === visit.id ? "bg-[var(--surface-brand-soft)] ring-1 ring-[var(--brand-500)]" : "hover:bg-[var(--surface-muted)]")}><p className="font-medium text-[var(--text-primary)]">{visit.title}</p><p className="mt-1 text-xs text-[var(--text-secondary)]">{visit.scheduled_at ? formatLocaleDate(language, visit.scheduled_at, { dateStyle: "medium", timeStyle: "short" }) : copy("visits.noScheduledTime")}</p></button>)}</div> : <EmptyState icon="event_available" title={copy("visits.emptyTitle")} description={copy("visits.emptyDescription")} />}
        </SurfaceCard>
