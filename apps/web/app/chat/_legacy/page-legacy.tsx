...
export default function ChatWorkspacePage() {
  ...
  // Clarifying-question gate for ambiguous deep/deep_beta queries (R12). When
  // set, the questions are shown and the research job is held until the user
  // answers or skips (R12.1, R12.5). `null` means no gate is pending.
  const [clarifyGate, setClarifyGate] = useState<{
    query: string;
  ...
  const submitMessage = async (
    message: string,
    submitOptions?: { clarifyResolved?: boolean; clarifyingAnswers?: Record<string, string> }
  ) => {
    ...
    // Clarifying-question gate (R12): before starting a deep/deep_beta research
    // job, ask the API whether the query is ambiguous. When it is, surface the
    // questions and do NOT start the job until the user answers or skips
    // (R12.1, R12.5). Unambiguous queries (or fast tier1 chat) start without
    // prompting (R12.4). A resolved decision (answer/skip) bypasses the gate.
    if (transport === "tier2_job" && !submitOptions?.clarifyResolved) {
      const clarification = await requestResearchClarification(message, {
        researchMode: selectedResearchMode,
        uiLanguage,
      });
      if (!canStartResearchJob(clarification.ambiguous, "pending")) {
        setClarifyGate({
          query: message,
    ...
    try {
      ...
      if (transport === "tier1_chat") {
        ...
        const { finalPayload } = await executeResearchTier2Job(message, {
          ...
          uiLanguage,
          clarifyingAnswers: submitOptions?.clarifyingAnswers,
          onJobCreated: (job) => {
  ...
  // R12.2: update a single clarifying answer as the user types.
  const onChangeClarifyAnswer = (questionId: string, value: string) => {
    setClarifyGate((prev) =>
      prev ? { ...prev, answers: { ...prev.answers, [questionId]: value } } : prev
    );
  };

  // R12.2: the user answered — start the deep job carrying the answers.
  const onSubmitClarifyAnswers = async () => {
    if (!clarifyGate || isSubmitting) return;
    const { query: clarifyQuery, answers } = clarifyGate;
    setClarifyGate(null);
    await submitMessage(clarifyQuery, { clarifyResolved: true, clarifyingAnswers: answers });
  };

  // R12.3: the user skipped — start the deep job with the original query.
  const onSkipClarify = async () => {
    if (!clarifyGate || isSubmitting) return;
    const { query: clarifyQuery } = clarifyGate;
    setClarifyGate(null);
    await submitMessage(clarifyQuery, { clarifyResolved: true });
  };

  // R12: abandoning the prompt simply clears the gate (no job starts, R12.5).
  const onDismissClarify = () => {
            ...
            <div className="mx-auto mb-2 w-full max-w-3xl px-3">
              ...
              >
                ...
                <div className="mt-3 flex flex-col gap-3">
                  {clarifyGate.questions.map((question) => (
                    <label key={question.id} className="flex flex-col gap-1">
                      <span className="text-[13px] font-semibold leading-5">{question.question}</span>
                      {question.rationale ? (
                        <span className="text-[11px] font-normal opacity-80">{question.rationale}</span>
                      ) : null}
                      <input
                        type="text"
