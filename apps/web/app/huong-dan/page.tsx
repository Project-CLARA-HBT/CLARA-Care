...
export default function GuidePage() {
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-10">
      <section className="rounded-2xl border border-[color:var(--shell-border)] bg-[var(--surface-panel)] p-6 shadow-sm">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Hướng dẫn sử dụng CLARA</h1>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link
            href="/"
            className="rounded-md border border-[color:var(--shell-border)] bg-[var(--surface-muted)] px-4 py-2 text-sm font-medium text-[var(--text-secondary)] hover:bg-[var(--surface-panel)]"
          >
            Về trang giới thiệu
          </Link>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {QUICK_START.map((item) => (
          <article key={item.role} className="rounded-xl border border-[color:var(--shell-border)] bg-[var(--surface-panel)] p-5 shadow-sm">
            <h2 className="text-base font-semibold text-[var(--text-primary)]">{item.role}</h2>
      ...
      <section className="rounded-2xl border border-[color:var(--shell-border)] bg-[var(--surface-panel)] p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-[var(--text-primary)]">Câu hỏi thường gặp</h2>
        <div className="mt-4 space-y-3">
          {FAQ.map((item) => (
            <details key={item.q} className="rounded-lg border border-[color:var(--shell-border)] bg-[var(--surface-muted)] p-4">
              <summary className="cursor-pointer text-sm font-semibold text-[var(--text-primary)]">{item.q}</summary>
              <p className="mt-2 text-sm leading-7 text-[var(--text-secondary)]">{item.a}</p>
            </details>
          ))}
        </div>
      </section>

      <section className="rounded-2xl border border-amber-300/70 bg-amber-100/70 p-6 dark:border-amber-400/40 dark:bg-[rgba(245,158,11,0.14)]">
