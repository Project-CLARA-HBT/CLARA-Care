  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        console.warn("Falling back to local AI transparency context:", err);
        return {
          data_classes_used: [
            {
              key: "demographics",
              name: isEn ? "Basic Demographics" : "Thông tin cơ bản",
              purpose: isEn ? "Contextualize health recommendations" : "Xác định ngữ cảnh cá nhân hóa",
              sensitive: false,
            },
            {
              key: "medications",
              name: isEn ? "Medications & Allergies" : "Đơn thuốc & Dị ứng",
              purpose: isEn ? "Evaluate DDI drug interactions" : "Kiểm tra tương tác thuốc DDI & FIDES",
              sensitive: true,
            },
            {
              key: "conditions",
              name: isEn ? "Medical History" : "Tiền sử bệnh lý",
              purpose: isEn ? "Clinical council reasoning" : "Hội đồng AI phân tích triệu chứng & phác đồ",
              sensitive: true,
            },
            {
              key: "vitals",
              name: isEn ? "Vital Signs" : "Chỉ số sinh hiệu",
              purpose: isEn ? "Critical symptom triage" : "Phát hiện dấu hiệu cảnh báo lâm sàng khẩn cấp",
              sensitive: true,
            },
          ],
          retention_policy: {
            days: 30,
            description: isEn
              ? "Health data retained for 30 days under PDPD and automatically deleted upon request."
              : "Dữ liệu được lưu trữ an toàn theo tiêu chuẩn PDPD và tự động thu hồi khi có yêu cầu.",
            auto_delete_enabled: true,
          },
          cot_zero_disclosure: {
            operates_without_cot: true,
            description: isEn
              ? "CLARA operates without permanent chain-of-thought reasoning storage (Zero-CoT)."
              : "CLARA không lưu trữ chuỗi suy luận nội bộ (Zero-CoT) trong nhật ký lưu trữ vĩnh viễn.",
            verified_guardrails: [
              "FIDES Verification",
            ...
            purposes: [
              {
                ...
                label: isEn ? "Medical Disclaimer" : "Tuyên bố miễn trừ y khoa",
                granted: true,
                locked: true,
              },
              {
                purpose: "ai_processing",
                label: isEn ? "AI-Assisted Processing" : "Xử lý hỗ trợ bởi Trí tuệ Nhân tạo",
                granted: true,
              },
              {
                purpose: "data_sharing",
                label: isEn ? "Family Care Sharing" : "Chia sẻ dữ liệu chăm sóc gia đình",
                granted: true,
              },
              {
                purpose: "analytics",
                label: isEn ? "Telemetry (Zero-PII)" : "Thống kê chất lượng dịch vụ (Zero-PII)",
                granted: true,
              },
            ],
          },
