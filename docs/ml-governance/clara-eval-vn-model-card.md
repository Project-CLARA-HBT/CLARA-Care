...
- Models/providers: no provider is invoked by the offline suite. The report
  records only the presence of approved runtime environment variable names,
...
- Rollback: disable the affected feature flag, select the previous approved
  model/prompt registry record, redeploy, then run `make eval-smoke`.
