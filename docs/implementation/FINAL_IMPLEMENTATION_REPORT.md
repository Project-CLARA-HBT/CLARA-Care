...
| PR-09 Scribe | implemented safety correction in code | `eaa749c0` removes automatic code/R69 and uncalibrated percentage. `0c6b268d` additionally requires consent for clinician-edit mutations and applies version-aware edit conflict handling. The ASR payload model is now resolved through a separate, fail-closed registry selection and disclosed as audio-model provenance, never as V4 Flash; text extraction remains independently task-routed. `b68b0899`, `39e04607`, `3ffba4e8`, `8c75a628` and `6a866b15` localize the Enterprise Scribe surface without changing its confirmation, signed-note/addendum or backend workflow semantics; redesigned-mobile Scribe presentation is covered by `b6f6c034`, `b9822d54` and `8f5ad61f`. The new endpoint and client tests were added but not run in this checkpoint. |
