...
- PHR OCR imports now require medical-disclaimer consent before upload and
  ...
  deployment.
- The PHR read-only share manager now resolves its create/revoke, scope,
  expiry, copy and error presentation through the same typed vi/en catalog.
  It does not change the server-enforced sharing-consent, capability, expiry or
  revocation boundary.
