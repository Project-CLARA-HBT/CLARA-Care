...
the individual CD steps that consume them. It also applies package-lock
overrides that resolve Next's optional Sharp dependency to `0.35.3` and all
PostCSS instances to `8.5.25`. The prior nested Next `postcss@8.4.31` entry is
removed from the lockfile. A fresh lockfile-only production
`npm audit --omit=dev --package-lock-only` on 2026-08-02 reported 0 critical,
0 high, 0 moderate and 0 low findings. This is local dependency evidence only;
GitHub's cross-ecosystem advisory total and a fresh deployed-image scan remain
...
8. GitHub's push response was observed moving from 130 to 100 and then to 59
   ...
   (latest observed: 1 critical, 28 high, 22 moderate, 8 low). A fresh local
   production lockfile audit is now clean (0 findings) after the global
   `postcss@8.5.25` override removed Next's nested vulnerable record. This does
   not establish that GitHub's cross-ecosystem advisory total is resolved;
   reconcile those advisories and scan the deployed image separately rather
   than forcing a framework downgrade.
