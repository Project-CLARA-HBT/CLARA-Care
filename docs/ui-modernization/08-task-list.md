...
| UI-04 | NFR-A11Y-002, NFR-A11Y-003 | Harden Field, Alert, SideSheet, ConfirmDialog and 44px targets. | Foundation | UI-02 | `components/ui/*`, dialogs | component focus/semantics tests | done |
...
| UI-09 | FR-LIFE-001–004 | Progressive LifeMap creation and episode presentation without truth changes. | Personal | UI-04/UI-06 | lifemap routes, guided flow | wizard/state/safety E2E | done |
...
| UI-15 | FR-EVID-001, FR-RES-001 | Evidence step disclosure and Source Hub browse/sync split. | Chat | UI-14 | evidence/research routes | release-gate/citation tests; four-viewport empty/result E2E | done |
| UI-16 | FR-COUNCIL-001–003 | Council case context, result hierarchy, canonical deep-well flow canvas and first-use empty state without false API failure. | Clinical | UI-04/UI-06 | council routes/components | safety/role/E2E | done |
| UI-17 | FR-SCRIBE-001–003 | Canonical Scribe stages, consent and sign semantics. | Clinical | UI-04/UI-06 | scribe page/components/libs | workflow/safety/E2E | done |
