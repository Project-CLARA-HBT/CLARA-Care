...
- Today real-data follow-up: `/lifemap/today` now returns a read-only, timezone-aware seven-day completion projection plus the scoped task's episode title/version. `/today` selects active, completed, caught-up, or first-time presentation from those values only. It never auto-completes a task or turns a pending draft into a conclusion. `completeLifeMapTask` sends `If-Match` when the server supplied a version. After E2E found a real 22–23px tablet/mobile overflow caused by raw icon-font fallback text, the mobile notification and More icons moved to the bundled SVG abstraction; four-viewport Today E2E is now 12/12. No migration or production deployment has occurred for this checkpoint yet.
