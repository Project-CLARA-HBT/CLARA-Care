...
| Phase-B execution boundary | Exact-model probe records family, JSON-contract support, usage availability, retries, non-streaming behavior, and endpoint hash. The benchmark entrypoint verifies clean live freeze state plus the probe SHA before provider access, binds artifacts to both, enforces two exact models and bounded FHIR inputs, and refuses to write below the Phase-A seal. | Implemented and tested with injected fake transports only; real probe/benchmark remain prohibited until freeze |
...
| Adversarial, property, leakage, secret gates | DSL property, leak/tamper, scoped secret scan, and hash-verified deterministic materialization of every minimal adversarial manifest into opaque case IDs, separate gold, and all nine solver packets. The knowledge-time ablation removes known-time metadata only from evidence visible at cutoff and cannot expose late evidence. | Partially complete: real application-boundary adversarial execution remains blocked |
...
| Clean SHA and sealed `implementation_freeze.json` | Freeze command scans structured command/result evidence in memory before writing, scans all tracked files, binds evidence to the tested SHA, and requires every named gate. The Phase-B probe independently validates the sealed run and rejects a dirty worktree, changed HEAD, or frozen-input inventory/hash drift before any client request. Twelve freeze/probe regressions pass. | Not complete: worktree is dirty; a full tracked scan reports seven credential-bearing historical/design files that need an explicit preservation/redaction decision |
...
1. Resolve the clean-worktree/committed-SHA boundary without discarding user
   changes.
2. Resolve or explicitly govern full-repository credential-like historical
   content and root lint/type debt.
