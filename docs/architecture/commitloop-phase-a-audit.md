...
| Clean SHA and sealed `implementation_freeze.json` | Current replacement freeze `17dd4b8c558c2ec0cb8c2572728093d9aa3ce914` is `COMPLETE` and `VALID`, freezes 47 transitive inputs, preserves the initial zero-call proof, discloses 1,990 Phase-B calls before replacement, and supersedes solver-v4 freeze `ac599d35d240dba83aa08ca3111575ccf23d25dd`. | Complete; replacement seal and artifact secret scan passed |
...
| Prospective solver-v5 cohort | Pre-registered 64 new subjects across eight balanced mechanism strata, one case per subject and no variants. The exact-model run completed 1,152/1,152 cells with zero errors/retries; construction accepted 64/64. Strict THSS reached 63/64 for Claude and 64/64 for Gemini. Claude beat Full History, Long Context, LWW, and Naive RAG after Holm; Gemini beat LWW and Naive RAG. | Six adjusted significant comparisons; controlled synthetic mechanism evidence only. Full detail: [`commitloop-confirmatory-v5-results.md`](commitloop-confirmatory-v5-results.md) |
| Prospective power resolution | The plan targeted 51 non-tied pairs, but significant comparisons contained 9 to 21 because most subject pairs tied. Exact sign tests remain valid and Holm-significant, while the pre-declared 80% power target was not met. | Statistically significant but not fully powered confirmation; independent tie-rate-adjusted replication required |
...
5. Pre-register and freeze a larger independent cohort with explicit power
   planning; do not reuse either completed cohort to chase significance. The
   next replication must account prospectively for the observed tie rates.
