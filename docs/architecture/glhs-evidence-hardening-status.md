...
| eICU Demo acquisition/adapter | IN_PROGRESS | Resumable atomic fetch, provider-SHA256 verifier and offset-preserving selected-table adapter; partial-only raw directory fails closed | Resume, partial-state, provider-checksum/tamper and adapter fixture tests | Source/license record; raw transfer remains local and incomplete | Official ZIP must finish, verify all packaged checksums, normalize, freeze and run source-derived structural tasks |
