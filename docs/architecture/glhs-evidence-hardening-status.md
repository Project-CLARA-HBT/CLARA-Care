...
| CMS DE-SynPUF OMOP local sources | PARTIAL | Full and 100K entries are distinct; streaming OMOP adapter covers eight event tables and rejects corrupt temporary objects before hashing | Registry/rejected-file tests plus OMOP provenance/time/minimization test | 100K source and normalization freezes; raw/normalized rows ignored | Full distribution has a corrupt extra LZO object; conversion lineage and canonical checksum unresolved |
| Synthea Coherent local source | PARTIAL | Full CRC/hash freeze and minimized FHIR-bundle adapter; CSV/DICOM/DNA modality inventory is aggregate-only | ZIP integrity, modality-minimization and provenance tests | Source and normalization freezes; normalized rows ignored | Provider checksum and modality linkage/semantic validation remain pending |
...
- eICU source metadata was frozen from clean SHA
  `18aad3988ba70ccd2cc4b50037ed18dc21ef7765`; manifest payload SHA-256 is
  `169411ad4edb49ac4d10c9ffcb51952403ef8ccd15a92602d06f3c7346e186e2`.
  The instrumented rerun was byte-identical and measured 24.764845 seconds,
  21,814.67 records/second, peak RSS 65,016 KiB and 5.7550x storage
  amplification. This is one local host run, not a general performance claim.
- eICU normalized aggregate evidence was frozen from clean SHA
  `cd973c83f93e31ed472abaf16d3e46bd7a19ae34`; manifest payload SHA-256 is
...
- The SynPUF OMOP 100K source freeze verified 17 gzip tables (914,865,701
  ...
  streaming OMOP adapter then emitted 39,573,534 deterministic gzip common
  records for 90,217 source subjects. Every emitted record has a source-valid
  date/datetime or explicit unknown, all knowledge times remain unknown, and
  no time was estimated. Normalized output SHA-256 is
  `69cef106...`; the tracked aggregate freeze payload is `40c2c0a2...`.
  Metrics are adapter/reproducibility evidence only, not clinical truth.
- Synthea Coherent passed full local ZIP CRC/inventory verification (2,488
  entries, 15,111,284,460 uncompressed bytes; source inventory SHA
  `144a6c34...`) and its minimized adapter emitted 1,297,901 records from
  1,278 subject bundles. Modalities were counted without retaining DICOM/DNA
  payloads (16 CSV, 298 DICOM, 889 DNA, 1,280 FHIR members); all knowledge
...
- SyntheticMass FHIR v1 source metadata was frozen from clean SHA
  ...
  1,307,771 FHIR bundles and 2,711,037 nested members with zero unsafe members.
  Manifest payload SHA-256 is
  `384e9fc5669aceea0070cb6a11ee621f9e63f298a87893312bffe4073c8443cd`;
  canonical checksum status remains `NOT_PROVIDED`, and no normalization or
  clinical claim is implied.
