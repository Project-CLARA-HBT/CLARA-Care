...
| G. Corrective evaluation iteration | in progress | Error-slice analysis found benchmark-contract defects rather than a defensible architecture comparison: timeliness gold used cutoff instead of decisive completion time, omitted grace semantics, packets omitted due/grace and contradiction relation, predicate generation lacked its allowed projection, and the two-event cohort could not distinguish all nine methods. Local repairs align the oracle with runtime reconciliation, restore required observability, expand the freeze inventory, and introduce an eight-subject controlled mechanism cohort with temporal-class and retrieval/history pressure. No additional router call is allowed before a new clean freeze. |
