...
| 11 | Research verifier | partial | claim/citation tracing exists and synchronous result release now requires a valid verifier state/version/summary/rows contract; cited prose with missing, malformed, unavailable or skipped verification safely abstains. Explicitly retracted external records are filtered before final evidence selection. The watermark backfill now uses the existing scheduler/orchestrator composition lazily behind the independent default-off `RAG_BACKFILL_ENABLED` kill switch, and records per-source sanitized failures without stopping siblings. Reviewed RAG gold set and fresh regression evidence are unavailable |
