...
| 06 | Hybrid task/risk router | partial | The primary chat semantic boundary now returns an extra-forbidden Pydantic decision contract (`action`, reason, emergency, task, bounded confidence and registry-owned model identity) rather than a free provider dictionary. Emergency output is normalized only to the deterministic fast path; contract failure, timeout or any unexpected key falls back to the existing deterministic emergency/legal floor. Governed V4 paths serve safe chat intents and read-only LifeMap asks, while a default-off, redacted Encoder-SLM shadow adapter remains unevaluated and cannot alter deterministic safety/authorization decisions. Source tests are added but unrun. |
