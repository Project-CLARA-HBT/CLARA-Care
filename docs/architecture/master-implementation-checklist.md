...
| 02 | i18n foundation | partial | typed vi/en catalog, parity and shell literal checks; authenticated shell, task-first surfaces and public landing are catalog-backed, while broader domain migration remains |
...
| 06 | Hybrid task/risk router | partial/pre-existing | semantic closed-schema primary with deterministic safety fallback plus a default-off, redacted Encoder-SLM shadow adapter; no externally provisioned/evaluated encoder is installed |
...
| 10 | Council hybrid shadow | partial/pre-existing | structured intake and ablation path; the fixed-weight shadow is now explicitly `rule_shadow`, never a neural model or probability, while the provisioned LLM/specialist evaluation remains external |
...
| 15 | Mobile parity | partial/pre-existing | unified mobile/locale/consent paths exist; a canonical static consumer terminology source now drives web and Unified mobile labels, while device E2E and broader feature parity remain |
