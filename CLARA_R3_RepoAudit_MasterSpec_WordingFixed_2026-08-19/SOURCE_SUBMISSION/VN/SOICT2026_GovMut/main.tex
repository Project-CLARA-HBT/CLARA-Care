...
The protocol frozen under run identifier \texttt{govmut-soict-2026-final-v2} executed all 45 core mutants across 16 method/seed combinations on revision \texttt{ab877e04} (720 executions), alongside the expanded 23,040-execution domain matrix. Unmutated baseline preflight passed all slots with zero false kills and zero infrastructure failures.

\section{Results}
Table~\ref{tab:res} details the comparative evaluation across the four strategies on the core corpus and the expanded systematic domain benchmark.
...
\label{tab:res}
\centering\small
\begin{tabularx}{\textwidth}{p{0.24\textwidth}c c c}
\toprule
...
M3 Combined GLHS & \textbf{20 (44.4\%)} & \textbf{1,440 / 1,440} & \textbf{100.0\%} ($p < 10^{-50}$ vs.\ M0)\\
\bottomrule
\end{tabularx}
\end{table}
