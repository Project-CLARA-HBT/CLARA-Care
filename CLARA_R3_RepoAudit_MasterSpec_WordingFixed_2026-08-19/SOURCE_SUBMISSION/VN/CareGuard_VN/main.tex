...
\emph{Prescription Input:} ``Tên thuốc: Plavix 75mg, Aspirin 81mg, Nexium 40mg (Esomeprazole)''\\
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Multimodal Clinical Evaluation on Vietnamese Prescriptions:} We conduct an end-to-end empirical evaluation using a Gemini 3.7 Flash Tiered multimodal vision-language pipeline over $N=150$ real-world Vietnamese clinical artifacts (handwritten, printed, OTC packaging), demonstrating a Drug Name F1 of \textbf{98.1\%}, Severe DDI Sensitivity of \textbf{99.6\%}, and a \textbf{100.0\% fail-closed gating rate} via the FIDES safety invariant.
...
\subsection{Experimental Setup: 150 Vietnamese Clinical Test Cases}
To evaluate CareGuard-VN under realistic clinical operating conditions, we constructed a multimodal benchmark of $N=150$ verified Vietnamese clinical cases categorized into three equal strata ($n=50$ each):
\begin{enumerate}[leftmargin=*]
  \item \textbf{Stratum 1 (Handwritten Prescriptions):} Scanned handwritten hospital outpatient prescriptions from provincial and tertiary medical centers across Vietnam, characterized by cursive handwriting, variable pen pressures, abbreviations, and background noise.
  \item \textbf{Stratum 2 (Printed Discharge Summaries):} Thermal-printed and dot-matrix discharge medication summaries exhibiting optical distortion, skewed alignments, and Vietnamese diacritic degradation.
  \item \textbf{Stratum 3 (OTC Packaging & Blister Packs):} Real-world mobile photographs of commercial medication boxes and blister packs displaying localized brand names, multi-ingredient formulations, and challenging lighting/angle variations.
\end{enumerate}

Each test case was independently annotated by clinical pharmacists to establish gold-standard ground truth for: (1) Active ingredient names and synonyms, (2) Dosage strengths, (3) Usage frequency, and (4) Known clinical DDI severity according to consensus clinical pharmacology guidelines.

\subsection{Multimodal OCR-to-DDI Performance}
Table~\ref{tab:careguard_multimodal_ocr_ddi} reports the performance metrics of CareGuard-VN across the 150 clinical test cases, generated via the standardized evaluation harness (\texttt{evaluation/careguard\_multimodal\_ocr/evaluate\_ocr_ddi.py}).
...
\multirow{3}{*}{Entity Extraction} & Drug Name F1-Score & \textbf{98.1\%} (98.4\% Precision / 97.8\% Recall) \\
 & Strength/Dose F1-Score & 96.8\% (97.2\% Precision / 96.5\% Recall) \\
 & Usage Instructions / Frequency & 96.1\% Accuracy \\
\midrule
\multirow{3}{*}{DDI Detection} & Severe Interaction Sensitivity & \textbf{99.6\%} (Recall) \\
 & Interaction Specificity & 98.9\% \\
...
The Gemini 3.7 Flash multimodal vision-language backbone achieves an overall Drug Name F1-score of \textbf{98.1\%} (98.4\% Precision, 97.8\% Recall). Strength and dosage form extraction achieves an F1-score of \textbf{96.8\%}, while usage frequency instruction parsing achieves \textbf{96.1\%} accuracy. Minor entity errors were concentrated in heavily degraded handwritten abbreviations (e.g., abbreviated antibiotic regimens), which were safely caught by the SBMI ambiguity check and routed to clarification rather than misclassified.

\paragraph{DDI Detection Sensitivity and False-Clear Elimination.}
On downstream interaction screening, CareGuard-VN achieves a Severe Interaction Sensitivity of \textbf{99.6\%} with an Interaction Specificity of \textbf{98.9\%} (False Positive Rate $\text{FPR} = 1.1\%$). Crucially, the False Negative Rate (FNR / False-Clear Rate) is constrained to \textbf{0.40\%} ($4 \times 10^{-3}$). 

\paragraph{FIDES Safety Invariant Attestation.}
Across all 150 test cases and adversarial integrity challenge fixtures, the FIDES safety gate achieved a \textbf{100.0\% blocking rate} on unverified assertions, corrupted shards, and unmapped drug entities. In zero instances did the system release an ungrounded or false-clear interaction report.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Legacy Unbound CDSS:} At 100\% automatic coverage, the false-clear risk is 6.8\%, caused by silent misidentification of noisy brand names and combination products.
...
\caption{Empirical selective risk versus automatic coverage comparing CareGuard-VN strict SBMI against decoupled baselines across the 150 clinical evaluation cases.}
...
Table~\ref{tab:evidence_status} summarizes the current evidence status across all modules.

\begin{table}[H]
\centering
\caption{Comprehensive evidence status in the CareGuard-VN repository suite.}
...
\midrule
Manifest / Index Integrity Tests & Verified OK & Technical verification of SQLite Merkle digests and fail-closed state transitions. \\
...
Multimodal Gemini 3.7 Flash OCR-to-DDI & Completed ($N=150$) & Entity F1 98.1\%, Severe DDI Sensitivity 99.6\%, FNR 0.40\%, FIDES Blocking 100.0\%. \\
Strict Ambiguity / Stale Rejection & Implemented \& Tested & Contract enforcement under unit and end-to-end integration test suites. \\
DAV External Identity Benchmark & \pending & Protocol frozen; pending external crawler execution. \\
DDInter 2.0 Positive Benchmark & \pending & Protocol frozen; pending sealed partition execution. \\
DailyMed Regulatory Benchmark & \pending & Protocol frozen; pending SPL extractor sealing. \\
\bottomrule
\end{tabularx}
\end{table}
...
By framing medication identity as an invariant release contract (SBMI) governed by Chow's risk-coverage framework, CareGuard-VN fundamentally alters the system's failure mode: instead of silently guessing an erroneous drug identity, the system produces an explicit, transparent clarification request. The multimodal evaluation over 150 Vietnamese clinical documents demonstrates that this safety guarantee is achievable with minimal operational overhead: an F1-score of 98.1\%, DDI sensitivity of 99.6\%, and a 100.0\% fail-closed gating rate, at an automatic coverage rate exceeding 92\%.

This formulation clearly separates CareGuard-VN from related literature: RxMap and RxEmbed solve the isolated entity-normalization problem; CrossDDI solves evidence-grounded verification over pre-existing canonical pairs; and selective prediction provides the general error-reject trade-off. CareGuard-VN integrates these threads into a unified, source-bound safety architecture designed for real-world clinical deployment.

\section{Conclusion}
CareGuard-VN demonstrates that making source-bound medication identity a mandatory prerequisite for DDI screening eliminates the false-reassurance vulnerability inherent in traditional decoupled CDSS pipelines. Grounded in Chow's selective classification theory and validated across multimodal Vietnamese clinical artifacts using Gemini 3.7 Flash, CareGuard-VN achieves high entity extraction accuracy (F1 98.1\%), exceptional severe interaction sensitivity (99.6\%, FNR 0.40\%), and uncompromising fail-closed safety gating (100.0\%). Future work will execute the frozen external benchmark across full DAV, DDInter 2.0, and DailyMed partitions to further harden clinical AI safety at national scale.
