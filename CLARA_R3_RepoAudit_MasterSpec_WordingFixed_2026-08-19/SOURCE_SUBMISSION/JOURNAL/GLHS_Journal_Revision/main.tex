...
Prior work has already established that durable agent effects require commit-time authority validation (CommitGuard, Santos-Grueiro 2026)~\cite{temporaryauth} and that stateful policies must be serialized with the effects they govern (Provenact, Peng \& Wu 2026~\cite{statefulgov}; MemTX, Li et al. 2026~\cite{memtx}). GLHS does not claim to discover stale authorization or transactional agent memory. Instead, it targets a narrower healthcare-specific compound invariant: a persistent clinical proposal must remain bound to the exact task-minimized, bitemporal health disclosure actually consumed during inference, including its clinical evidence, consent event, policy epoch, and state dependencies, and those coordinates must be revalidated atomically with the resulting mutation. Relative to CommitGuard, this adds a reconstructable clinical disclosure and evidence contract; relative to policy-state serializability, it integrates that contract with longitudinal health-state versioning and provenance. Our claims are limited to the snapshot-bound paths and deployment conditions directly exercised by the accompanying PostgreSQL evidence.
