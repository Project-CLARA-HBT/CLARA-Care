...
\paragraph{Agentic Transactions and Concurrency Control (Generic Workflow Isolation vs.\ Healthcare Governance-Snapshot Atomicity).}
Recent systems research extends classical transaction concepts to multi-agent LLM systems. Works on Agentic Transaction~\cite{agentictransaction2026} and Adaptive Concurrency Control for Agentic Transactions (ATCC)~\cite{atcc2026} formalize isolation levels, abort/rollback mechanisms, and dependency-graph serialization for agentic tool use and multi-step reasoning workflows. While Agentic Transaction and ATCC address general agentic workflow concurrency and execution hazards, they treat underlying state as generic unstructured memory registers or arbitrary tool-call endpoints without domain-specific clinical constraints. They do not enforce healthcare-specific bitemporal valid-time and knowledge-time semantics ($t_v \le \text{valid\_cutoff} \land t_k \le \text{known\_cutoff}$), fail to bind cryptographic Merkle disclosure proofs of task-minimized health records to durable writeback proposals, and omit monotonic consent event epochs ($e_{\text{consent}}$) necessary to eliminate A-B-A revocation blindness.
...
We formalize the core serializability guarantee of the GLHS Dual-Layer Barrier through Theorem 1 (formal mathematical proof in Appendix~\ref{sec:proofs}). Cryptographic snapshot integrity relies on standard Second-Preimage Resistance and Collision Resistance of SHA-256 with canonical Merkle digest verification (where computing an altered evidence payload $E' \ne E$ satisfying $\operatorname{MerkleDigest}(E') = d_H$ occurs with negligible probability $\le \operatorname{negl}(\lambda)$). In GST admission, shared governance read locks (\texttt{FOR SHARE}) are acquired on domain policy and subject consent anchors during proposal verification, allowing concurrent proposal admissions while blocking exclusive governance write locks (\texttt{FOR UPDATE} for consent revocations or policy epoch advances). Row-level exclusive locks (\texttt{SELECT ... FOR UPDATE}) are acquired on clinical entity partitions according to the unified phantom-safe canonical Two-Phase Locking (2PL) lock hierarchy:
\begin{equation}
...
\end{equation}
where $\text{PolicyAnchor}(d)$ locks the stable policy epoch anchor for domain $d$ (shared for GST read revalidation, exclusive for policy epoch advances), $\text{ProfileAndConsentAnchor}(u)$ locks patient $u$'s profile root and monotonic consent event epoch $e_{\text{consent}}$ (shared for GST read revalidation, exclusive for consent lifecycle modifications), $\text{EntityPartitions}(u, k)$ locks accessed clinical entity partitions in $\operatorname{Deps}(P)$ ordered lexicographically by canonical key ($\prec_{\text{lex}}$), and $\text{LeaseState}(l)$ locks active authorization leases. This strict canonical lock ordering prevents deadlock cycles and guarantees conflict serializability across concurrent clinical mutations and governance modifications, while permitting disjoint entity partition writes to execute concurrently in parallel with $0.00\%$ false-stale aborts.

\paragraph{Theorem 1 (No-TOCTOU Disclosure Serializability under Phantom-Safe Canonical 2PL and Monotonic Consent Epochs).}
\emph{Let $\mathcal{S}$ be any interleaved execution schedule of snapshot disclosures, concurrent clinical state transitions $\mathcal{T}_{\text{state}}$, governance modifications $\mathcal{T}_{\text{gov}}$ (including consent grants, revocations, re-grants, role updates, and policy epoch advances), and persistent proposal admissions $\mathcal{T}_{\text{admit}}$ governed by Strict Two-Phase Locking (SS2PL) with shared governance read locks vs.\ exclusive governance write locks under the canonical lock acquisition hierarchy $\mathcal{L}_{\text{canonical}}$ (Eq.~\ref{eq:canonical_lock_hierarchy}):
$$\mathcal{L}_{\text{canonical}} = \text{PolicyAnchor}(d) \prec \text{ProfileAndConsentAnchor}(u) \prec_{\text{lex}} \text{EntityPartitions}(u, k) \prec \text{LeaseState}(l).$$
If a proposal $P$ compiled from snapshot $\mathcal{H}$ at disclosure timestamp $t_{\text{disclose}}(\mathcal{H})$ with monotonic consent event epoch $e_{\text{consent}}(\mathcal{H})$ and policy epoch $v_{\pi}(\mathcal{H})$ commits at transaction timestamp $t_{\text{commit}}(P)$, then for all entity partitions $e_k \in \operatorname{Deps}(P)$ and governing coordinates $\langle v_{\pi}, e_{\text{consent}} \rangle$, there exists no intermediate committed transition $\mathcal{T} \in \mathcal{T}_{\text{state}} \cup \mathcal{T}_{\text{gov}}$ affecting $e_k$ or $\langle v_{\pi}, e_{\text{consent}} \rangle$ whose commit timestamp $t_{\mathcal{T}}$ satisfies $t_{\text{disclose}}(\mathcal{H}) < t_{\mathcal{T}} < t_{\text{commit}}(P)$. Furthermore:
\begin{enumerate}[label=(\roman*)]
  \item Disjoint entity partition writes ($\operatorname{Deps}(P_1) \cap \operatorname{Deps}(P_2) = \emptyset$) acquire compatible shared governance read locks on $\text{PolicyAnchor}(d)$ and $\text{ProfileAndConsentAnchor}(u)$ and non-overlapping exclusive partition locks, committing in parallel with $0.00\%$ false-stale aborts;
  \item Because consent event epochs $e_{\text{consent}}$ advance strictly monotonically upon every consent lifecycle transition ($e_{\text{consent}}^{(t_2)} > e_{\text{consent}}^{(t_1)}$ for grants, revocations, and re-grants $A \to B \to A$), the protocol is immune to A-B-A revocation blindness; and
  \item The serialization graph $SG(\mathcal{S})$ is acyclic, ensuring strict conflict serializability without deadlocks or phantom anomalies.
\end{enumerate}}
...
2. Digest Tampering & Mutated snapshot payload under stale Merkle digest & 32/32 & 0/32 & 32 & $4.66 \times 10^{-10}$\\
...
8. Lineage Forgery & Forged downstream review proposal provenance & 32/32 & 0/32 & 32 & $4.66 \times 10^{-10}$\\
\midrule
...
\paragraph{Fair normalized benchmark comparison under identical clinical safety functions.}
To evaluate GLHS against state-of-the-art transactional memory, access control, and healthcare systems, we conducted an architectural and semantic simulation study providing a fair, normalized benchmark comparison of formal concurrency control and governance invariants across six simulated peer paradigms (Table~\ref{tab:baseline_comparison}) under identical hardware, concurrency ($W=16$ concurrent simulated workers), and standardized clinical safety functions across $N=500$ clinical workloads (comprising single-entity updates, multi-entity cross-domain updates, dynamic TOCTOU revocation races, severe DDI exposures, and disjoint parallel slots). Rather than containerizing full third-party proprietary server binaries, our evaluation rigorously executes in-memory semantic models capturing the precise protocol, locking, conflict-detection, and authorization-witness semantics of each paradigm under identical workload conditions:
\begin{enumerate}
...
\centering
\small
\caption{Fair Normalized Semantic Comparison of Concurrency \& Governance Paradigms under Identical Clinical Safety Functions ($N=500$ Workloads, $W=16$ Concurrent Simulated Workers). \textit{Note:} Throughput (TPS) and p95 latency reflect in-memory semantic simulation dispatch and conflict-evaluation times on identical hardware.}
...
The Governed State Transition (GST) admission protocol enforces Strict Two-Phase Locking (SS2PL) with shared governance read locks vs.\ exclusive governance write locks under the unified phantom-safe canonical lock hierarchy:
\begin{equation}
\mathcal{L}_{\text{canonical}} = \text{PolicyAnchor}(d) \prec \text{ProfileAndConsentAnchor}(u) \prec_{\text{lex}} \text{EntityPartitions}(u, k) \prec \text{LeaseState}(l),
...
where:
\begin{enumerate}
    \item $\text{PolicyAnchor}(d)$ locks the stable policy epoch anchor for clinical domain $d$ (acquired in shared mode \texttt{FOR SHARE} by GST read revalidation, and in exclusive mode \texttt{FOR UPDATE} by policy epoch updates);
    \item $\text{ProfileAndConsentAnchor}(u)$ locks the patient root profile and monotonic consent event epoch record ($e_{\text{consent}}$) (acquired in shared mode \texttt{FOR SHARE} by GST read revalidation, and in exclusive mode \texttt{FOR UPDATE} by consent grants, revocations, and re-grants);
...
\paragraph{2. Phantom-Safe Synchronization on Stable Lock Roots (Shared Governance Read Locks vs.\ Exclusive Write Locks).}
In standard predicate locking, a phantom anomaly occurs when a concurrent transaction inserts a new record satisfying a query predicate after a reader has evaluated that predicate. Under GLHS, every entity partition $e_k$ is subordinate to the immutable patient profile root $\text{ProfileAndConsentAnchor}(u)$, and every governance rule is subordinate to $\text{PolicyAnchor}(d)$. Any transaction attempting to insert a new entity partition, mutate an existing partition, revoke consent, or publish an updated policy epoch must first acquire an exclusive lock on the corresponding stable parent anchor ($\text{ProfileAndConsentAnchor}(u)$ or $\text{PolicyAnchor}(d)$) before mutating partition tables or epoch counters. GST proposal admission transactions acquire shared read locks on these stable anchors, which allows multiple concurrent GST transitions to read the governance state without blocking one another, while mutually excluding any concurrent exclusive governance modification $\mathcal{T}_{\text{gov}}$. Because both GST writers ($\mathcal{T}_{\text{admit}}$) and governance mutators ($\mathcal{T}_{\text{gov}}$) synchronize on these fixed, stable lock roots, no phantom insertions or unobserved predicate mutations can occur between disclosure timestamp $t_1 = t_{\text{disclose}}(\mathcal{H})$ and commit timestamp $t_3 = t_{\text{commit}}(P)$.

\paragraph{3. Parallel Execution of Disjoint Partition Writes with 0\% False-Stale Aborts.}
Let $\mathcal{T}_1$ and $\mathcal{T}_2$ be two concurrent proposal admission transactions for subject $u$ with mutually disjoint dependency partition sets:
\begin{equation*}
\operatorname{Deps}(P_1) \cap \operatorname{Deps}(P_2) = \emptyset.
\end{equation*}
Under $\mathcal{L}_{\text{canonical}}$, both $\mathcal{T}_1$ and $\mathcal{T}_2$ acquire shared governance read locks on $\text{PolicyAnchor}(d)$ and $\text{ProfileAndConsentAnchor}(u)$. Because shared read locks are mutually compatible, neither transaction blocks the other on the root anchors. Next, $\mathcal{T}_1$ and $\mathcal{T}_2$ acquire row-level exclusive locks on their respective entity partition sets:
\begin{equation*}
\text{Locks}(\mathcal{T}_1) = \{ e_i \in \operatorname{Deps}(P_1) \}, \quad \text{Locks}(\mathcal{T}_2) = \{ e_j \in \operatorname{Deps}(P_2) \}.
\end{equation*}
Because $\operatorname{Deps}(P_1) \cap \operatorname{Deps}(P_2) = \emptyset$, the sets of exclusive row locks are disjoint ($\text{Locks}(\mathcal{T}_1) \cap \text{Locks}(\mathcal{T}_2) = \emptyset$). Consequently, neither transaction experiences lock contention or waits on the other. When $\mathcal{T}_1$ commits and increments $V(e_i)$ for $e_i \in \operatorname{Deps}(P_1)$, it does not mutate any partition in $\operatorname{Deps}(P_2)$. Therefore, the version vector $V(e_j)$ for all $e_j \in \operatorname{Deps}(P_2)$ remains identical to the base version read at disclosure ($V(e_j)_{\text{current}} == v_s(e_j)$), satisfying $\operatorname{StateCurrent}(P_2)$. Since neither transaction mutated governance anchors, $\operatorname{GovernanceCurrent}(P_2)$ also holds. Thus, $\mathcal{T}_2$ commits successfully without aborting, achieving a $0.00\%$ false-stale abort rate in parallel execution.

\paragraph{4. Proof of No-TOCTOU Invariant and A-B-A Blindness Immunity by Contradiction.}
...
\begin{itemize}
    ...
    At $t_3$, following canonical order $\mathcal{L}_{\text{canonical}}$, GST acquires shared read locks on $\text{PolicyAnchor}(d)$ and $\text{ProfileAndConsentAnchor}(u)$.
...
\paragraph{5. Acyclicity of the Serialization Graph $SG(\mathcal{S})$.}
