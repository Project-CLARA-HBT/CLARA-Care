...
\paragraph{Database Concurrency and Serializable Snapshot Isolation (SSI).}
In transactional database systems, Serializable Snapshot Isolation (SSI)~\cite{cahill2008,cahill2009} enforces conflict serializability under snapshot isolation by dynamically detecting cycles in the dependency graph through read--write anti-dependencies ($rw$-antidependencies / \texttt{SIREAD} locks). While SSI prevents write-skew and phantom anomalies on physical database records, it operates strictly over raw data-item reads and writes within the execution lifetime of a single database transaction. SSI is fundamentally unaware of the asynchronous, multi-second cognitive reasoning cycles of LLM agents. Consequently, SSI does not bind the multi-second LLM inference context to dynamic external governance lifecycle transitions---such as patient dynamic consent revocation, tenant policy epoch advances, or clinician role reassignments---that occur outside the database transaction boundary between prompt-time data retrieval and post-inference writeback.

\paragraph{Consistent Authorization Systems and Policy Engines.}
In distributed access control, systems such as Google Zanzibar~\cite{zanzibar2019} provide consistent snapshot authorization by evaluating Access Control List (ACL) checks at a consistent point-in-time snapshot token (zookie), ensuring causal consistency across globally replicated authorization metadata. Similarly, declarative policy engines such as Open Policy Agent (OPA)~\cite{opa} and AWS Cedar~\cite{cedar} evaluate fine-grained attribute-based and role-based policies against structured context payloads. However, these authorization frameworks decouple access-check evaluation from the underlying clinical mutation transaction: they do not atomically commit the authorization decision with the resulting clinical mutation and a verifiable cryptographic Merkle evidence proof in a single database transaction. This architectural separation leaves an open Time-of-Check to Time-of-Use (TOCTOU) vulnerability where governing consent, clinical evidence, or policy rules can be altered during LLM inference or clinical review without invalidating the pending mutation at commit time.

\paragraph{GLHS Positioning: Governance-Snapshot Atomicity.}
GLHS is therefore not positioned as a generic transaction primitive or a standalone authorization engine. Instead, GLHS defines its contribution precisely as \textbf{governance-snapshot atomicity}: atomically coupling clinical entity mutations with stable consent, policy, and authorization coordinates in one database transaction and binding the resulting snapshot into an auditable, cryptographically verifiable commitment. Where CommitGuard~\cite{temporaryauth} checks whether authority remains valid at durability and Provenact~\cite{statefulgov} checks policy serializability, GLHS answers a domain-specific clinical informatics requirement: whether the model's persistent proposal is admissible \emph{given the exact governed health information, consent epoch, policy version, and Merkle evidence from which the proposal was derived}.
...
\end{equation}
where $\text{PolicyAnchor}(d)$ locks the stable policy epoch anchor for domain $d$, $\text{ProfileAndConsentAnchor}(u)$ locks patient $u$'s profile root and dynamic consent epoch $v_c$, $\text{EntityPartitions}(u, k)$ locks accessed clinical entity partitions in $\operatorname{Deps}(P)$ ordered lexicographically by canonical key ($\prec_{\text{lex}}$), and $\text{LeaseState}(l)$ locks active authorization leases. This strict canonical lock ordering prevents deadlock cycles and guarantees conflict serializability across concurrent clinical mutations and governance modifications.

\paragraph{Theorem 1 (No-TOCTOU Disclosure Serializability under Phantom-Safe Canonical 2PL).}
\emph{Let $\mathcal{S}$ be any interleaved execution schedule of snapshot disclosures, concurrent clinical state transitions $\mathcal{T}_{\text{state}}$, governance modifications $\mathcal{T}_{\text{gov}}$ (e.g., consent revocations, role changes, policy epoch advances), and persistent proposal admissions $\mathcal{T}_{\text{admit}}$ governed by Strict Two-Phase Locking (SS2PL) with canonical lock acquisition hierarchy $\mathcal{L}_{\text{canonical}}$ (Eq.~\ref{eq:canonical_lock_hierarchy}):
$$\mathcal{L}_{\text{canonical}} = \text{PolicyAnchor}(d) \prec \text{ProfileAndConsentAnchor}(u) \prec_{\text{lex}} \text{EntityPartitions}(u, k) \prec \text{LeaseState}(l).$$
If a proposal $P$ compiled from snapshot $\mathcal{H}$ at disclosure timestamp $t_{\text{disclose}}(\mathcal{H})$ commits at transaction timestamp $t_{\text{commit}}(P)$, then for all entity partitions $e_k \in \operatorname{Deps}(P)$ and governing policy/consent coordinates $\langle v_{\pi}, v_c \rangle$, there exists no intermediate committed transition $\mathcal{T} \in \mathcal{T}_{\text{state}} \cup \mathcal{T}_{\text{gov}}$ affecting $e_k$ or $\langle v_{\pi}, v_c \rangle$ whose commit timestamp $t_{\mathcal{T}}$ satisfies $t_{\text{disclose}}(\mathcal{H}) < t_{\mathcal{T}} < t_{\text{commit}}(P)$. Furthermore, the serialization graph $SG(\mathcal{S})$ is acyclic, ensuring strict conflict serializability without deadlocks or phantom anomalies.}
...
\newpage
\appendix
\section{Formal Proof of Theorem 1 (No-TOCTOU Disclosure Serializability under Phantom-Safe Canonical 2PL)}
\label{sec:proofs}
...
\begin{enumerate}
    ...
    \item $\text{ProfileAndConsentAnchor}(u)$ locks the patient root profile and dynamic consent epoch $v_c$;
...
\paragraph{3. Proof of No-TOCTOU Invariant by Contradiction.}
Assume for the sake of contradiction that $P$ commits at $t_3 = t_{\text{commit}}(P)$, but there exists an intermediate transition $\mathcal{T} \in \mathcal{T}_{\text{state}} \cup \mathcal{T}_{\text{gov}}$ committed at $t_2$ ($t_1 < t_2 < t_3$) affecting some entity partition $e_k \in \operatorname{Deps}(P)$ or modifying governance coordinates $\langle v_{\pi}, v_c \rangle$.
...
\begin{itemize}
    ...
    If $\mathcal{T}$ committed a consent revocation or advanced the governing policy epoch at $t_2$, it locked $\text{ProfileAndConsentAnchor}(u)$ or $\text{PolicyAnchor}(d)$ and updated the epoch counters: $v_c(t_2) > v_c(t_1)$ or $v_{\pi}(t_2) > v_{\pi}(t_1)$.
