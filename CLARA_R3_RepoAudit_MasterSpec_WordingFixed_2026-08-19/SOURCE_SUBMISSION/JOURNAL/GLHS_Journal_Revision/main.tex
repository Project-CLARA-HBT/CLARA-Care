...
\title{\textbf{GLHS: A Co-Versioned Disclosure-to-Commit Governance Contract for Longitudinal Health AI}}
...
\subsection{Formal Theorems: Serializability and Deadlock Freedom}
We formalize the core correctness guarantees of the GLHS Dual-Layer Barrier and Entity DAG OCC protocol through three theorems (formal mathematical proofs are provided in Appendix~\ref{sec:proofs}):

\paragraph{Theorem 1 (No-TOCTOU Disclosure Serializability).}
\emph{Let $\mathcal{S}$ be any interleaved execution schedule of read operations, concurrent clinical state transitions $\mathcal{T}_{\text{state}}$, governance modifications $\mathcal{T}_{\text{gov}}$ (e.g., consent revocations, role changes, policy epoch advances), and persistent proposal admissions $\mathcal{T}_{\text{admit}}$. If a proposal $P$ generated from snapshot $\mathcal{H}$ commits at transaction timestamp $t_{\text{commit}}$, then for all entity partitions $v \in \operatorname{Deps}(P)$, there does not exist any intermediate committed transition $\mathcal{T} \in \mathcal{T}_{\text{state}} \cup \mathcal{T}_{\text{gov}}$ affecting $v$ with commit timestamp $t_{\mathcal{T}}$ satisfying $t_{\text{disclose}}(\mathcal{H}) < t_{\mathcal{T}} < t_{\text{commit}}(P)$.}

\paragraph{Theorem 2 (Deadlock-Free Multi-Partition Concurrency).}
\emph{Let $\prec_{\text{lex}}$ be the strict total lexicographical order on entity partition keys $\mathcal{K} = \mathcal{U} \times \mathcal{D} \times \mathcal{S}$. For every transaction $T_i$ committing proposal $P_i$, let locks be acquired in the canonical order $\mathcal{L}_i = \operatorname{sort}_{\prec_{\text{lex}}}(\operatorname{Deps}(P_i))$. Then the dynamic Wait-For Graph $\operatorname{WFG}(\mathcal{S})$ contains no directed cycles for any interleaved execution $\mathcal{S}$, guaranteeing that the system is strictly deadlock-free ($\operatorname{DeadlockRate} = 0.0\%$).}

\paragraph{Theorem 3 (Soundness and Completeness of Deterministic State Barrier).}
\emph{The Layer 1 Non-LLM Reconciliation Kernel evaluates due-window arithmetic, bitemporal intersection $\operatorname{Valid}(e) \cap \operatorname{Know}(e)$, and mutual exclusivity rules over finite clinical taxonomies $\mathcal{X}$ with $100\%$ precision and recall (zero false-positive contradictions, zero false-negative due-date breaches), isolating the epistemic LLM layer from temporal hallucination.}
...
\toprule
\textbf{Attack Family} & \textbf{Adversarial Injection Mechanism} & \textbf{No-Binding} & \textbf{GLHS Bound} & \textbf{Discordance ($b$)} & \textbf{Exact McNemar $p$}\\
\midrule
1. Wrong Snapshot ID & Swapped foreign snapshot ID with valid digest & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
2. Digest Tampering & Mutated snapshot payload under stale Merkle root & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
3. Undisclosed Evidence & Inserted unread clinical lab/DDI evidence into proposal & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
4. Cross-Snapshot Substitution & Replaced snapshot from identical state version & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
5. Temporal Expiry & Submitted proposal past snapshot validity window & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
6. Minimized Evidence Swap & Substituted broader evidence set than authorized & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
7. Coordinate Tampering & Mismatched actor/role/purpose/task coordinates & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
8. Post-Review Lineage Forge & Forged downstream review proposal signature & 32/32 Admitted & 0/32 Admitted & 32 & $4.657 \times 10^{-10}$\\
\midrule
\textbf{Total Adversarial ($N=256$)} & \textbf{Combined across all 8 attack vectors} & \textbf{256/256 (100\%)} & \textbf{0/256 (0.0\%)} & \textbf{256} & $\mathbf{1.727 \times 10^{-77}}$\\
\textbf{Valid Controls ($N=64$)} & \textbf{Clean, un-tampered authorized commitments} & \textbf{64/64 (100\%)} & \textbf{64/64 (100\%)} & \textbf{0} & \textbf{1.0 (Exact Match)}\\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\textbf{Writers ($W$)} & \textbf{Monolithic False-Stale} & \textbf{Entity DAG Disjoint} & \textbf{Entity DAG Overlapping} & \textbf{Deadlock Rate} & \textbf{Throughput (TPS)}\\
\midrule
1 & 0.00\% (0/1) & 0.00\% (0/1) & 0.00\% (0/1) & 0.0\% & 32.4\\
2 & 50.00\% (1/2) & 0.00\% (0/2) & 50.00\% (1/2) & 0.0\% & 61.8\\
4 & 75.00\% (3/4) & 0.00\% (0/4) & 75.00\% (3/4) & 0.0\% & 118.2\\
8 & 87.50\% (7/8) & 0.00\% (0/8) & 87.50\% (7/8) & 0.0\% & 224.5\\
16 & 93.75\% (15/16) & 0.00\% (0/16) & 93.75\% (15/16) & 0.0\% & 412.1\\
32 & 96.88\% (31/32) & 0.00\% (0/32) & 96.88\% (31/32) & 0.0\% & 746.8\\
64 & 98.44\% (63/64) & 0.00\% (0/64) & 98.44\% (63/64) & 0.0\% & 1,280.4\\
128 & 99.22\% (127/128) & 0.00\% (0/128) & 99.22\% (127/128) & 0.0\% & 2,154.0\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Head-to-head empirical comparison across clinical AI frameworks}
We evaluated GLHS against three representative paradigms on the same PostgreSQL 16 suite: (1) Vanilla Multi-Agent RAG (stateless generation), (2) Stateful Agent Memory (MemGPT / Letta), and (3) FHIR REST Optimistic Locking with ETag (HAPI FHIR / Smile CDR).

\begin{table}[H]
\centering
\small
\caption{Empirical Paradigm Comparison: Evaluating TOCTOU Safety, Concurrency, and Temporal Soundness across four clinical AI architectures.}
\label{tab:baseline_comparison}
\begin{tabularx}{\textwidth}{l c c c c c}
\toprule
\textbf{Architecture} & \textbf{TOCTOU Violation} & \textbf{False-Stale ($W=16$)} & \textbf{Temporal Math Accuracy} & \textbf{P95 Latency} & \textbf{Formal Proof}\\
\midrule
Vanilla Multi-Agent RAG & 100.0\% (256/256) & 0.0\% (Unchecked) & 78.4\% (LLM Error 21.6\%) & 0.0 ms & None\\
MemGPT / Letta & 100.0\% (256/256) & 0.0\% (Unchecked) & 81.2\% (LLM Error 18.8\%) & 2.4 ms & None\\
FHIR REST (ETag / If-Match) & 75.0\% (192/256) & 93.75\% (15/16) & N/A (Client Delegated) & 3.1 ms & None\\
\textbf{GLHS v2 (This Work)} & \textbf{0.00\% (0/256)} & \textbf{0.00\% (0/16)} & \textbf{100.0\% (Kernel AST)} & \textbf{10.5 ms} & \textbf{Theorems 1--3}\\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Later synthetic checks and a null comparator result}
A later sealed Claude-only synthetic run used 384 independent subjects, nine context conditions, and 3,456 solver cells. The prespecified subject-level contrast between Strict THSS and full authorized history was null: 70 wins, 73 losses, and 241 ties; mean Strict-minus-full-history effect $-0.0078125$; 95\% bootstrap CI $[-0.0677083,0.0520833]$; exact two-sided sign-test $p=0.8672499071$. We therefore do not claim universal model-level superiority of Strict THSS over complete authorized history. The run also contained 220 fail-closed malformed outputs and is reported as descriptive synthetic software evidence rather than clinical validation.
...
\newpage
\appendix
\section{Formal Mathematical Proofs of Theorems}
\label{sec:proofs}

\subsection{Proof of Theorem 1 (No-TOCTOU Disclosure Serializability)}
\begin{proof}
Let $\mathcal{H} = \operatorname{THSS}(u, a, r, \phi, \tau, \vec{v}_0, id_H, d_H, E_H)$ be compiled at timestamp $t_1 = t_{\text{disclose}}(\mathcal{H})$ with cryptographic Merkle digest $d_H = \operatorname{MerkleRoot}(E_H \cup \vec{v}_0 \cup \Phi_{\text{policy}} \cup \Sigma_{\text{consent}})$.
Let $P = \langle u, a, r, \phi, \tau, v_s, v_{\pi}, v_c, id_H, d_H, \Delta, \rho \rangle$ be the proposal generated by Layer 2, with dependency partition set $\operatorname{Deps}(P) \subset \mathcal{V}_u$.

Assume for the sake of contradiction that $P$ is committed at $t_3 = t_{\text{commit}}(P)$, but there exists an intermediate transition $\mathcal{T}$ committed at $t_2$ ($t_1 < t_2 < t_3$) affecting some partition $e_k \in \operatorname{Deps}(P)$ or modifying governance coordinates $\langle \Phi, \Sigma \rangle$.
By definition of a committed state transition in PostgreSQL MVCC under read-committed or serializable isolation:
\begin{enumerate}
    \item If $\mathcal{T}$ advanced the clinical state of partition $e_k$, the monotonic version counter was updated: $V(e_k)_{t_2} = V(e_k)_{t_1} + 1 > V(e_k)_{t_1}$.
    \item At $t_3$, GST executes within an atomic database transaction and acquires a row-level lock \texttt{SELECT ... FOR UPDATE} on $e_k$.
    \item GST evaluates the predicate $\operatorname{StateCurrent}(P)$, which verifies that the base version declared in $P$ matches the currently locked row: $V(e_k)_{\text{current}} == v_s(e_k)$.
    \item Since $V(e_k)_{\text{current}} = V(e_k)_{t_2} > v_s(e_k)$, the condition $\operatorname{StateCurrent}(P)$ evaluates to $\text{False}$.
    \item Analogously, if $\mathcal{T}$ revoked consent or advanced policy epoch, $\operatorname{GovernanceCurrent}(P)$ evaluates to $\text{False}$ upon re-reading the committed governance state under the lock.
    \item If the caller attempts to bypass the check by mutating $P$, the cryptographic binding check $\operatorname{Bound}(P, \mathcal{H})$ verifies $d_H(P) == \operatorname{RecomputeMerkleRoot}(id_H)$, which fails with $\text{False}$ upon payload mismatch.
\end{enumerate}
Therefore, $\operatorname{Commit}(P)$ in Eq.~\ref{eq:commit} evaluates to $\text{False}$, and $P$ is unconditionally aborted. This contradicts the assumption that $P$ was committed at $t_3$. Hence, no intermediate conflicting transition $\mathcal{T}$ can exist. $\blacksquare$
\end{proof}

\subsection{Proof of Theorem 2 (Deadlock-Free Canonical Partition Ordering)}
\begin{proof}
Let $\mathcal{K} = \mathcal{U} \times \mathcal{D} \times \mathcal{S}$ be the set of all possible entity partition keys $\langle \text{profile\_id}, \text{domain}, \text{semantic\_key} \rangle$.
Define the relation $\prec_{\text{lex}}$ on $\mathcal{K}$ as the standard lexicographical order on the 3-tuple of strings. Since string comparison is a strict total order, $\prec_{\text{lex}}$ is irreflexive, antisymmetric, and transitive.

For any set of concurrent transactions $\{ T_1, T_2, \dots, T_k \}$ attempting to commit proposals $\{ P_1, P_2, \dots, P_k \}$, each transaction $T_i$ acquires locks strictly in the sequence $\mathcal{L}_i = \operatorname{sort}_{\prec_{\text{lex}}}(\operatorname{Deps}(P_i))$.

Suppose there exists a deadlock cycle in the Wait-For Graph $\operatorname{WFG}(\mathcal{S})$:
$$T_1 \to T_2 \to \dots \to T_m \to T_1 \quad (m \ge 2)$$
An edge $T_i \to T_j$ implies that $T_i$ is waiting for a lock on key $K_b \in \mathcal{K}$ held by $T_j$, while $T_i$ already holds a lock on key $K_a \in \mathcal{K}$.
Because $T_i$ acquires locks strictly in ascending $\prec_{\text{lex}}$ order, $T_i$ must have requested $K_a$ before $K_b$, which requires $K_a \prec_{\text{lex}} K_b$.
Simultaneously, for $T_j$ to hold $K_b$ while waiting on another key $K_c$, it must hold that $K_b \prec_{\text{lex}} K_c$.

Traversing the entire cycle $T_1 \to T_2 \to \dots \to T_m \to T_1$ yields the chain:
$$K_1 \prec_{\text{lex}} K_2 \prec_{\text{lex}} \dots \prec_{\text{lex}} K_m \prec_{\text{lex}} K_1$$
By transitivity, $K_1 \prec_{\text{lex}} K_1$, which contradicts the irreflexivity of the strict total order $\prec_{\text{lex}}$.
Therefore, no directed cycle can exist in $\operatorname{WFG}(\mathcal{S})$, and the protocol is provably deadlock-free. $\blacksquare$
\end{proof}

\subsection{Proof of Theorem 3 (Soundness \& Completeness of Layer 1 State Barrier)}
\begin{proof}
Let $\mathcal{X}$ be a finite, decidable clinical taxonomy and let $\mathcal{E}$ be a set of bitemporal clinical assertions.
The Layer 1 Deterministic Reconciliation Kernel evaluates constraints $\psi \in \Psi$ defined over the AST DSL.
\begin{itemize}
    \item \textbf{Soundness:} For every due window $[t_{\text{start}}, t_{\text{end}}]$ and reference observation $o$, the interval calculation $t_{\text{due}} = t_{\text{ref}} + \delta$ is an arithmetic mapping over discrete POSIX UTC timestamps. Since arithmetic over bounded integers in Python/PostgreSQL is exact and deterministic, $\mathcal{K}_{\text{det}}$ admits a state as valid if and only if all temporal and logical predicates hold. Hence, the false positive rate of constraint validation is 0.
    \item \textbf{Completeness:} Given a ground-truth contradiction (e.g., mutually exclusive active medication codes $c_1 \cap c_2 \neq \emptyset$), the AST evaluator scans all pairwise assertions in $O(|\mathcal{E}|^2)$ time. Because the taxonomy table is finite and closed under database relational queries, every conflicting pair is identified and tagged as \texttt{CONFLICTED} with probability 1.0 (zero false negatives).
\end{itemize}
Thus, Layer 1 isolates the epistemic arbiter (Layer 2 LLM) by guaranteeing $100\%$ sound and complete evaluation of deterministic clinical rules. $\blacksquare$
\end{proof}
