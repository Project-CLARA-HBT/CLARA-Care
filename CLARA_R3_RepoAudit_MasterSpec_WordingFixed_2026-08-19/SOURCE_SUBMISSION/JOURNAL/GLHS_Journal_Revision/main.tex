...
\subsection{Comparative evaluation against transactional and healthcare concurrency baselines}
\paragraph{Failure of unconstrained agent memory frameworks.}
Unconstrained LLM memory frameworks (such as Vanilla Multi-Agent RAG and MemGPT / Letta) inherently fail clinical ACID requirements: because they treat working memory as an unstructured prompt context without transactional isolation boundaries, they delegate temporal interval arithmetic to the stochastic model (yielding a $18.8\%$--$21.6\%$ error rate) and permit ungrounded, asynchronous write operations that result in $100\%$ TOCTOU corruption vulnerability ($256/256$ adversarial schedule breaches) under concurrent care-team updates.

\paragraph{Head-to-head empirical comparison across transactional baselines.}
We benchmarked GLHS against state-of-the-art transactional and healthcare concurrency architectures on PostgreSQL 16: (1) FHIR REST Optimistic Locking with ETag (\texttt{If-Match}), (2) MemTX (transactional belief commit and snapshot isolation~\cite{memtx}), and (3) CommitGuard (commit-time temporary authorization boundaries~\cite{temporaryauth}).

\begin{table}[H]
\centering
\footnotesize
\caption{Empirical Comparison across Transactional AI Architectures: Evaluating TOCTOU Safety, Concurrency False-Stale Aborts, DDI Leakage, and Latency.}
...
\toprule
\textbf{Architecture} & \textbf{TOCTOU Breach} & \textbf{False-Stale ($W=16$)} & \textbf{Severe DDI Leaks} & \textbf{p95 Latency}\\
\midrule
FHIR REST (ETag / \texttt{If-Match}) & 75.0\% (192/256) & 93.8\% (15/16) & Delegated (100\%) & 3.1 ms\\
MemTX (Snapshot Isolation) & 5.0\% (13/256) & 12.5\% (2/16) & 20.0\% (No Layer 1) & 6.4 ms\\
CommitGuard (Temporal Auth) & 0.0\% (0/256) & 6.25\% (1/16) & 7.6\% (Generic) & 8.2 ms\\
\textbf{GLHS v2 (This Work)} & \textbf{0.00\% (0/256)} & \textbf{0.00\% (0/16)} & \textbf{0.0\% (Layer 1 Gate)} & \textbf{10.5 ms}\\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Proof of Theorem 2 (Cryptographic Non-Forgeability and Bounded-Commit Security of THSS Merkle Digest)}
\begin{proof}
Let $\mathcal{H}_{\text{orig}} = \operatorname{THSS}(u, a, r, \phi, \tau, \vec{v}_0, id_H, d_H, E_H)$ be compiled at time $t_1$ with Merkle root $d_H = \operatorname{MerkleRoot}(E_H \cup \vec{v}_0 \cup \Phi_{t_1} \cup \Sigma_{t_1})$ using a cryptographically collision-resistant hash function $\mathcal{H}_{\text{hash}}: \{0,1\}^* \to \{0,1\}^\lambda$ (where $\lambda = 256$ for SHA-256).
...
To succeed in $\operatorname{GST\_Commit}(P^*, t_2) == \text{True}$, $\mathcal{A}$ must simultaneously satisfy:
\begin{enumerate}
    \item \textbf{Snapshot Identity Verification:} $\operatorname{Bound}(P^*, \mathcal{H}_{\text{orig}}) \iff d_H(P^*) == \operatorname{RecomputeMerkleRoot}(id_H)$.
    \item \textbf{Causal Precedence Invariant:} $V(e_k)_{t_2} == v_s^*(e_k)$ for all $e_k \in \operatorname{Deps}(P^*)$.
    \item \textbf{Governance Invariant:} $\Sigma_{t_2} == v_c^* \land \Phi_{t_2} == v_\pi^*$.
\end{enumerate}

Suppose $V(e_k)_{t_2} > v_s(e_k)$ or $\Sigma_{t_2} \neq \Sigma_{t_1}$.
If $\mathcal{A}$ sets $v_s^*(e_k) = V(e_k)_{t_2}$ or $v_c^* = \Sigma_{t_2}$ to satisfy conditions (2) or (3), the recomputed Merkle root yields $d_H^* = \operatorname{MerkleRoot}(E_H \cup \vec{v}_{t_2} \cup \Phi_{t_2} \cup \Sigma_{t_2}) \neq d_H$.
Thus, condition (1) requires finding $E^*, \vec{v}^*, \Phi^*, \Sigma^*$ distinct from $E_H, \vec{v}_0, \Phi_{t_1}, \Sigma_{t_1}$ such that:
...
$$\Pr[\mathcal{A} \text{ finds collision in } \mathcal{H}_{\text{hash}}] \le \frac{q_H^2}{2^\lambda} \le \operatorname{negl}(\lambda)$$
where $q_H$ is the number of random oracle queries.
Therefore, the probability of admitting an ungrounded or temporally decoupled state transition is bounded by:
...
Hence, GLHS guarantees cryptographic non-forgeability and bounded-commit security. $\blacksquare$
\end{proof}
