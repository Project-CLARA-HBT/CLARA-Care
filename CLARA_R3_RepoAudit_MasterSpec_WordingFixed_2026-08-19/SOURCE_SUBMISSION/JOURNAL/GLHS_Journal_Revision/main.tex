...
\toprule
\textbf{Architecture} & \textbf{TOCTOU Breach} & \textbf{False-Stale ($W=16$)} & \textbf{Temporal Math} & \textbf{P95 Latency}\\
\midrule
Vanilla Multi-Agent RAG & 100.0\% (256/256) & 0.0\% (Unchecked) & 78.4\% (Error 21.6\%) & 0.0 ms\\
MemGPT / Letta & 100.0\% (256/256) & 0.0\% (Unchecked) & 81.2\% (Error 18.8\%) & 2.4 ms\\
FHIR REST (ETag / If-Match) & 75.0\% (192/256) & 93.8\% (15/16) & N/A (Delegated) & 3.1 ms\\
\textbf{GLHS v2 (This Work)} & \textbf{0.00\% (0/256)} & \textbf{0.00\% (0/16)} & \textbf{100.0\% (AST)} & \textbf{10.5 ms}\\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Biostatistical non-inferiority and TOST equivalence analysis}
...
\toprule
\textbf{Dimension} & \textbf{Statistical / Systems Metric} & \textbf{Value} & \textbf{Biostatistical Decision} \\
\midrule
...
Overall TOST & $p_{\text{TOST}} = \max(p_1, p_2)$ & $p_{\text{TOST}} = 9.44 \times 10^{-8}$ & \textbf{Equivalence Confirmed ($p < 0.001$)} \\
95\% Confidence Interval & $[\hat{\Delta} \pm t_{0.975} \cdot SE]$ & $[-1.233\%, -0.330\%]$ & \textbf{Strictly Contained in $[-\delta, +\delta]$} \\
...
\subsection{Later synthetic checks and non-inferiority context-minimization trade-offs}
...
\begin{proof}
Let $\mathcal{H} = \operatorname{THSS}(u, a, r, \phi, \tau, \vec{v}_0, id_H, d_H, E_H)$ be compiled at timestamp $t_1 = t_{\text{disclose}}(\mathcal{H})$ with cryptographic Merkle digest:
\begin{equation*}
...
\subsection{Proof of Theorem 2 (Deadlock-Free Canonical Partition Ordering and Dynamic Wound-Wait Protocol)}
\begin{proof}
Let $\mathcal{K} = \mathcal{U} \times \mathcal{D} \times \mathcal{S}$ be the set of all entity partition keys:
\begin{equation*}
K = \langle \text{profile\_id}, \text{domain}, \text{semantic\_key} \rangle.
\end{equation*}
...
\textbf{Case 1: Static Pre-Declared Lock Batches.}
For any set of concurrent transactions $\{ T_1, T_2, \dots, T_k \}$ committing proposals $\{ P_1, P_2, \dots, P_k \}$, each transaction $T_i$ acquires locks strictly in the sequence $\mathcal{L}_i = \operatorname{sort}_{\prec_{\text{lex}}}(\operatorname{Deps}(P_i))$.
...
Traversing the entire cycle yields $K_1 \prec_{\text{lex}} K_2 \prec_{\text{lex}} \dots \prec_{\text{lex}} K_m \prec_{\text{lex}} K_1$.
By transitivity, $K_1 \prec_{\text{lex}} K_1$, which contradicts the irreflexivity of $\prec_{\text{lex}}$. Thus, no static cycle can exist.

\textbf{Case 2: Dynamic Multi-Hop Entity Expansion under Wound-Wait.}
When multi-agent reasoning chains dynamically discover and request entity coordinates during execution, transactions acquire locks dynamically. Let each transaction $T_i$ be assigned a unique, monotonically increasing logical timestamp $ts(T_i)$ at admission.
Under the Wound-Wait (WW) non-preemptive priority rule:
\begin{enumerate}
    \item When an older transaction $T_{\text{old}}$ ($ts(T_{\text{old}}) < ts(T_{\text{young}})$) requests a coordinate held by a younger transaction $T_{\text{young}}$, $T_{\text{old}}$ \emph{wounds} $T_{\text{young}}$. $T_{\text{young}}$ is transitioned to state \texttt{WOUNDED}, its lease is preempted, and $T_{\text{young}}$ aborts cleanly without blocking $T_{\text{old}}$.
    \item When a younger transaction $T_{\text{young}}$ requests a coordinate held by an older transaction $T_{\text{old}}$, $T_{\text{young}}$ enters state \texttt{WAITING} and blocks on $T_{\text{old}}$.
\end{enumerate}
Therefore, any directed edge $T_a \to T_b$ in $\operatorname{WFG}(\mathcal{S})$ can exist \emph{if and only if} $ts(T_a) > ts(T_b)$ (younger waiting for older).
Any wait chain must form a strictly monotonically decreasing timestamp sequence:
$$ts(T_1) > ts(T_2) > \dots > ts(T_m)$$
A directed cycle $T_1 \to \dots \to T_m \to T_1$ requires $ts(T_1) > ts(T_1)$, an impossibility on real-valued timestamps.
Hence, $\operatorname{WFG}(\mathcal{S})$ is strictly acyclic across both static batching and dynamic multi-hop expansion, guaranteeing $0$ deadlocks under arbitrary concurrent workloads. $\blacksquare$
\end{proof}
