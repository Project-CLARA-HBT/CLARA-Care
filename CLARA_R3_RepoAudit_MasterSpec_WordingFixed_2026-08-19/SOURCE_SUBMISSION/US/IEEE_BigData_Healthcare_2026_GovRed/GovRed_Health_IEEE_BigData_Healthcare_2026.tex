...
\begin{abstract}
Stateful clinical AI systems operating over longitudinal healthcare big data repeatedly query historical patient context and subsequently emit persistent state modifications. However, in distributed clinical workflows, patient state, bitemporal valid/knowledge horizons, dynamic consent, governing policy, role permissions, purpose constraints, cache state, or snapshot validity can drift across the disclosure-to-commit interval ($\Delta t = t_{\text{commit}} - t_{\text{disc}}$). Rooted in Snodgrass's foundational bitemporal data systems framework, GovRed-Health establishes an empirical benchmark for authorization drift in stateful clinical big data environments. Each matched logical schedule issues a task-bounded governed disclosure, injects precise bitemporal state or governance mutations, and tests persistence through the application, authorization, Redis cache, PostgreSQL, and cryptographic audit boundary. The evaluation isolates scalable longitudinal governance mechanisms while enforcing zero-PHI (Protected Health Information) leakage via high-entropy sentinel verification. In a sealed isolated run ($N=270$ synthetic schedules per arm, 210 primary executable schedules), the prespecified non-safe composite was 57.1\% (120/210; 95\% CI: 0.504--0.636) for Unbound, 42.9\% (90/210; 95\% CI: 0.364--0.496) for State-Version-Only, 42.9\% (90/210; 95\% CI: 0.364--0.496) for Snapshot-Bound-State-Only, and 14.3\% (30/210; 95\% CI: 0.102--0.197) for GLHS-Strict. Exact paired McNemar testing confirmed a decisive advantage for Strict over Unbound ($b=90$, $c=0$, $p=1.62\times 10^{-27}$). All 30 Strict residuals were concurrent stale-state schedules with indeterminate linearization; serial authorization-drift families produced zero confirmed invalid Strict commits. Prohibited disclosure and sentinel PHI leakage remained strictly 0/270 (0.00\%) in every arm. The benchmark provides a reproducible systems-assurance framework for longitudinal clinical big data architectures.
\end{abstract}
