...
Property-based testing, model-based testing, state-machine testing, mutation testing, and access-control testing are established techniques~\cite{quickcheck,utting,jiaharman,pretschner,xu,nist192}. Recent agent-memory and software-engineering work also applies property-based assurance, bounded exploration, and mutation to AI-related systems~\cite{memtx,swemutation}. GovMut-Health therefore does not introduce a new testing method.

Instead, it contributes a health-AI governance fault model in which seeded defects are tied to real enforcement sites and explicit contract invariants. The benchmark asks which combinations of established tests detect failures in state freshness, consent/policy freshness, subject and purpose binding, snapshot validity, retries, provenance, atomicity, and audit reconstruction. The mutant is the inferential object; repeated generated traces and seeds support reproducibility rather than inflating the sample size.
