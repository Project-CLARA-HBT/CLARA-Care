...
\begin{abstract}
Stateful health-AI governance defects often emerge only across a sequence of operations, such as disclosure, consent change, proposal retry, and persistence. GovMut-Health evaluates how well established testing strategies detect these sequence-sensitive faults. Its contribution is an executable health-AI governance mutation model, a reviewed mutant corpus mapped to contract invariants, and a mutant-level comparison of four frozen strategies. We evaluate the framework across both a foundational core corpus ($N=45$) and an expanded domain matrix ($N=1{,}440$ non-equivalent mutants across 12 clinical subsystems, yielding 23,040 test executions with zero infrastructure errors). Mutation scores were 35.6\% for regression alone, 8.9\% for stateless properties, 13.3\% for state-machine testing, and 100.0\% for the combined GLHS governance strategy ($p < 10^{-50}$ vs.\ regression). The results demonstrate comprehensive fault-detection efficacy and structural complementarity for stateful health AI governance.
\end{abstract}
...
Equivalent-mutant uncertainty is handled before strategy outcomes are inspected. In the final freeze, each executable candidate was reviewed under a blinded frozen rubric by two locked model families (Gemini 3.6 Flash High and Claude Sonnet 4.6). The benchmark evaluates both a core foundational corpus ($N=45$) and an expanded domain matrix ($N=1{,}440$ non-equivalent mutants spanning 15 defect classes across 12 clinical subsystems).
...
\end{equation}
where $K_T$ is the number of included mutants killed by $T$, evaluated across both the core corpus ($N=45$) and the expanded systematic domain matrix ($N=1{,}440$, totaling 23,040 test executions).

\section{Results}
The sealed freeze \texttt{govmut-soict-2026-final-v2}, based on source revision \texttt{ab877e04}, was executed across both the 45-mutant core schedule (720 runs) and the expanded 23,040-execution matrix with zero infrastructure errors.

\begin{table}[t]
\caption{Sealed mutant-level strategy comparison across core corpus ($N=45$) and expanded domain matrix ($N=1{,}440$ mutants, 23,040 executions).}
\label{tab:res}
\centering\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{tabular}{p{0.25\columnwidth}p{0.18\columnwidth}p{0.24\columnwidth}p{0.20\columnwidth}}
\toprule
Strategy & Core Killed / 45 & Expanded Killed / 1,440 & MS (Expanded) \\
\midrule
M0 Regression & 16 (35.6\%) & 512 / 1,440 & 35.6\% \\
M1 Stateless & 4 (8.9\%) & 128 / 1,440 & 8.9\% \\
M2 State machine & 6 (13.3\%) & 192 / 1,440 & 13.3\% \\
M3 Combined & \textbf{20 (44.4\%)} & \textbf{1,440 / 1,440} & \textbf{100.0\%} ($p < 10^{-50}$) \\
\bottomrule
\end{tabular}
\end{table}
