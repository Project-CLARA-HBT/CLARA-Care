...
def test_execution_requires_explicit_isolated_attestation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FreezeError, match="govred_execution_requires_isolated_research_attestation"):
        execute(
...
def test_executed_row_requires_real_boundary_evidence(
    ...
    case = {"case_id": "case-1", "family": "cross_subject_retrieval", "oracle": {"sentinel_token": "S"}}
...
def test_executed_row_rejects_declared_arm_without_implementation_evidence(
    ...
    with pytest.raises(FreezeError, match="govred_adapter_arm_implementation_attestation_missing"):
        _executed(
            {"case_id": "case-1", "family": "cross_subject_retrieval", "oracle": {"sentinel_token": "S"}},
            "UNBOUND",
...
def test_executed_row_rejects_unverifiable_or_outside_artifact_binding(
    ...
    case = {"case_id": "case-1", "family": "cross_subject_retrieval", "oracle": {"sentinel_token": "S"}}
    observation = BoundaryObservation(
        status_code=403, response_body=b"denied", db_before_sha256="a" * 64,
        db_after_sha256="a" * 64, audit_reconstruction_complete=True,
        cache_index_revocation_failure=False, commit_occurred=False, latency_ms=1.0,
        availability_error=False,
    )
    implementation_ref, implementation_hash = _artifact(tmp_path, "arm.json", b"arm")
    value = {
        "isolated_attestation": True, "arm_name": "GLHS_STRICT", "observation": observation,
        "execution_id": "synthetic-execution", "normalized_outcome": "rejected",
        "boundary_path_attestation": {"http": True, "postgres": True, "cache": True, "audit": True},
        "observation_artifact_ref": "../outside.json", "observation_artifact_sha256": "a" * 64,
        "arm_implementation_attestation": {
            "name": "GLHS_STRICT", "bind_snapshot": True, "revalidate_state": True,
            "revalidate_governance": True, "research_only": True,
            "runtime_mode": "isolated_research_only", "production_defaults_unchanged": True,
            "implementation_artifact_ref": implementation_ref,
            "implementation_artifact_sha256": implementation_hash, "implementation_revision": "d" * 40,
        },
    }
