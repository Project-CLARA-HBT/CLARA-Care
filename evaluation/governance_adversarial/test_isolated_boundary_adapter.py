...
def test_adapter_marks_unsupported_family_as_not_run(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _configure(monkeypatch, tmp_path)
    assert adapter.adapter(case=_case("gst_bypass_prompt"), arm=_arm()) == {"run_status": "NOT_RUN"}


def _two_phase_request(rejection_code: str = "stale_state_version", audit_body: bytes = b'{"audit_reconstruction_complete":false}'):
    ...
    def request(_config, path, *, method="GET", body=None, token=None):
        ...
        if path.endswith("/arm"):
            return 200, json.dumps({"arm": "GLHS_STRICT", "bind_snapshot": True, "revalidate_state": True, "revalidate_governance": True}).encode(), False
        if path.endswith("/synthetic-commit-probe") and body is not None and body.get("phase") == "create":
            return 201, json.dumps({"arm": "GLHS_STRICT", "phase": "create", "probe_id": body.get("probe_id"), "proposal_public_id": "proposal-1", "snapshot_public_id": "snapshot-1"}).encode(), False
...
def test_cross_subject_retrieval_executes_two_phase_without_cache_claim(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _configure(monkeypatch, tmp_path)
    audit_body = json.dumps({
        ...
        "rejection_coordinates": {"proposal_public_id": "proposal-1", "snapshot_public_id": "snapshot-1"},
        "rejection_context": {"actor_user_id": 1, "purpose": "self_care", "task": "govred-isolated-synthetic-probe"},
    }).encode()
    request, calls = _two_phase_request(rejection_code="assertion_scope_forbidden", audit_body=audit_body)
    ...
    assert [call[2]["mutation"] for call in commit_calls] == ["subject_cross_replay", "subject_cross_replay"]
    assert result["mutation_class"] == "subject_cross_replay"
    assert result["normalized_outcome"] == "rejected"
    assert result["boundary_path_attestation"] == {"http": True, "postgres": True, "cache": False, "audit": True}
...
def test_purpose_mismatch_executes_two_phase_purpose_switch(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _configure(monkeypatch, tmp_path)
    audit_body = json.dumps({
        "commit_found": False,
        "transition_item_count": 0,
        ...
        "rejection_coordinates": {"proposal_public_id": "proposal-1", "snapshot_public_id": "snapshot-1"},
        "rejection_context": {"actor_user_id": 1, "purpose": "care_coordination", "task": "govred-isolated-synthetic-probe"},
    }).encode()
    request, calls = _two_phase_request(rejection_code="proposal_snapshot_purpose_mismatch", audit_body=audit_body)
    ...
    commit_calls = [call for call in calls if call[1].endswith("/synthetic-commit-probe")]
    assert [call[2]["phase"] for call in commit_calls] == ["create", "commit"]
    assert [call[2]["mutation"] for call in commit_calls] == ["purpose_switch_replay", "purpose_switch_replay"]
    assert result["mutation_class"] == "purpose_switch_replay"
    assert result["normalized_outcome"] == "rejected"
    assert result["boundary_path_attestation"] == {"http": True, "postgres": True, "cache": False, "audit": True}
    assert result["rejection_audit_event"]["rejection_reason_code"] == "proposal_snapshot_purpose_mismatch"
    assert result["rejection_audit_event"]["rejection_context"]["purpose"] == "care_coordination"


def test_policy_version_change_executes_two_phase(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _configure(monkeypatch, tmp_path)
    ...
    commit_calls = [call for call in calls if call[1].endswith("/synthetic-commit-probe")]
    assert [call[2]["phase"] for call in commit_calls] == ["create", "commit"]
    assert [call[2]["mutation"] for call in commit_calls] == ["policy_version_change", "policy_version_change"]
    assert result["mutation_class"] == "policy_version_change"
    assert result["normalized_outcome"] == "rejected"
    assert result["boundary_path_attestation"] == {"http": True, "postgres": True, "cache": False, "audit": True}


def test_boundary_attestation_derived_from_family_contracts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _configure(monkeypatch, tmp_path)
    ...
    cache_contract = {
        "revoked_consent_cache_index_reuse": {"http": True, "postgres": True, "cache": True, "audit": True},
        "derived_cache_persistence_after_revocation": {"http": True, "postgres": True, "cache": True, "audit": True},
    }
    for family, expected in cache_contract.items():
        assert adapter._boundary_path_attestation(family) == expected


def test_adapter_artifact_records_mutation_and_rejection_audit(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _configure(monkeypatch, tmp_path)
    audit_body = json.dumps({
        "commit_found": False,
        "transition_item_count": 0,
    ...
    request, _ = _two_phase_request(rejection_code="assertion_policy_mismatch", audit_body=audit_body)
    monkeypatch.setattr(adapter, "_request", request)
    monkeypatch.setattr(adapter, "_snapshot", lambda _: {"postgres_sha256": "b" * 64, "redis_sha256": "c" * 64, "audit_sha256": "d" * 64})
    ...
    assert artifact["rejection_audit_event"]["rejection_coordinates"]["proposal_public_id"] == "proposal-1"
    assert "response_body" not in artifact["observation"]
