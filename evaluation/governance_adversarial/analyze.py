...
def _exact_mcnemar_pvalue(discordant_a: int, discordant_b: int) -> float | None:
    ...
    if total == 0:
        return None
    lower_tail = (
        sum(math.comb(total, index) for index in range(min(discordant_a, discordant_b) + 1))
        / 2**total
    )
            ...
            aggregates.append(
                {
                    ...
                    "not_run_logical_cases": sum(
                        1
                        for row in rows
                        if row["family"] == family
                        and row["arm"] == arm
                        and row["run_status"] == "NOT_RUN"
                    ),
...
def _paired_comparisons(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    ...
    for family in FAMILIES:
        for left, right in (
            ("UNBOUND", "GLHS_STRICT"),
            ("STATE_VERSION_ONLY", "GLHS_STRICT"),
            ("SNAPSHOT_BOUND_STATE_ONLY", "GLHS_STRICT"),
        ):
            pairs = [
                outcomes
                for (row_family, _), outcomes in by_case.items()
                if row_family == family and left in outcomes and right in outcomes
            ]
            ...
            comparisons.append(
                {
                    ...
                    "left_only_failures": left_only,
                    "right_only_failures": right_only,
                    "exact_mcnemar_p_unadjusted": _exact_mcnemar_pvalue(left_only, right_only),
                }
            )
    return comparisons
...
def _holm_adjust(comparisons: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for scope in {str(item["reporting_scope"]) for item in comparisons}:
        ...
        for rank, (_, item) in enumerate(ordered):
            adjusted = min(
                1.0, max(previous, float(item["exact_mcnemar_p_unadjusted"]) * (total - rank))
            )
