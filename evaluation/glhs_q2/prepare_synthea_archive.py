                ...
                with tarfile.open(fileobj=nested_stream, mode="r|gz") as inner:
                    for member in inner:
                        ...
                        if scanned_bundles % progress_every_bundles == 0:
                            print(
                                "synthea_scan "
                                f"bundles={scanned_bundles} selected={len(selected)} "
                        ...
                        if int(hashlib.sha256(token.encode("ascii")).hexdigest(), 16) % selection_modulus == 0:
                            selected.append((token, episodes))

    if len(selected) < 100:
        raise ValueError(f"synthea_selected_cases_below_minimum:{len(selected)}")
    selected.sort(key=lambda item: item[0])
    rows: list[dict[str, object]] = []
    for index, (token, episodes) in enumerate(selected, start=1):
