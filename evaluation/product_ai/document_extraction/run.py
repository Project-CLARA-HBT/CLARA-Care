"""Runner for Document Extraction Benchmark suite."""
...
def run_benchmark(
    ...
    os.environ["CLARA_MODEL_ROUTE_TASK_LIFEMAP_TEXT_DRAFT_EXTRACTION_PROVIDER"] = provider
    os.environ["CLARA_MODEL_ROUTE_TASK_LIFEMAP_TEXT_DRAFT_EXTRACTION_MODEL"] = model

    doc_system_prompt = (
        "Trích xuất thông tin cấu trúc y tế chính xác sang JSON.\n"
        "Định dạng JSON cần có:\n"
        "- \"medications\": mảng các thuốc [{ \"name\": ..., \"active_ingredient\": ..., \"strength\": ..., \"dosage\": ..., \"instructions\": ... }]\n"
        "- \"measurements\": mảng các chỉ số xét nghiệm [{ \"analyte\": ..., \"value\": <số>, \"unit\": ..., \"reference_range\": ..., \"flag\": ... }]\n"
        "- \"diagnoses\": mảng chuỗi các chẩn đoán\n"
        "- \"admission_date\": chuỗi ngày nhập viện (YYYY-MM-DD)\n"
        "- \"discharge_date\": chuỗi ngày xuất viện (YYYY-MM-DD)\n"
        "Chỉ trả về JSON hợp lệ duy nhất, không kèm văn bản giải thích."
    )

    def _eval_single_case(case: TaskCase) -> tuple[CaseEvaluationResult, float]:
        req = ModelRequest(
            prompt=case.prompt,
            system_prompt=case.system_prompt or doc_system_prompt,
            task=ModelTask.LIFEMAP_TEXT_DRAFT_EXTRACTION,
        ...
        try:
            resp, _ = gateway.execute(ModelTask.LIFEMAP_TEXT_DRAFT_EXTRACTION, req)
    ...
    try:
        ...
        os.environ.pop("CLARA_MODEL_ROUTE_TASK_LIFEMAP_TEXT_DRAFT_EXTRACTION_PROVIDER", None)
        os.environ.pop("CLARA_MODEL_ROUTE_TASK_LIFEMAP_TEXT_DRAFT_EXTRACTION_MODEL", None)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Document Extraction Benchmark")
