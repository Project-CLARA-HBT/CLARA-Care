"""Runner for Vietnamese Clinical NLP Benchmark suite."""
...
def run_benchmark(
    ...
    os.environ["CLARA_MODEL_ROUTE_TASK_CLINICAL_LANGUAGE_EXTRACTION_PROVIDER"] = provider
    os.environ["CLARA_MODEL_ROUTE_TASK_CLINICAL_LANGUAGE_EXTRACTION_MODEL"] = model

    vn_system_prompt = (
        "Bạn là hệ thống xử lý ngôn ngữ y tế tiếng Việt CLARA NLP. Hãy phân tích yêu cầu lâm sàng và xuất JSON chính xác:\n"
        "- Với từ viết tắt: xuất đối tượng \"abbreviations\": {\"<viết_tắt>\": \"<dạng đầy đủ>\"}\n"
        "- Với phân tích phủ định: xuất mảng \"symptoms\": [{\"name\": \"<tên triệu chứng>\", \"negated\": true/false}]\n"
        "- Với chuẩn hóa thuật ngữ dân gian: xuất mảng \"terms\": [\"<thuật ngữ y khoa tương đương>\"]\n"
        "- Với chuẩn hóa không dấu: xuất trường \"normalized\": \"<cụm từ có dấu>\"\n"
        "- Với thuật ngữ Anh-Việt / code-mixing: xuất các trường \"procedure\", \"timing\", \"infection\" (true/false)\n"
        "- Với nhận diện biệt dược: xuất mảng \"medications\": [\"<tên thuốc / hoạt chất>\"]\n"
    ...
    def _eval_single_case(case: TaskCase) -> tuple[CaseEvaluationResult, float]:
        req = ModelRequest(
            prompt=case.prompt,
            system_prompt=case.system_prompt or vn_system_prompt,
            task=ModelTask.CLINICAL_LANGUAGE_EXTRACTION,
        ...
        try:
            resp, _ = gateway.execute(ModelTask.CLINICAL_LANGUAGE_EXTRACTION, req)
    ...
    try:
        ...
        os.environ.pop("CLARA_MODEL_ROUTE_TASK_CLINICAL_LANGUAGE_EXTRACTION_PROVIDER", None)
        os.environ.pop("CLARA_MODEL_ROUTE_TASK_CLINICAL_LANGUAGE_EXTRACTION_MODEL", None)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Vietnamese NLP Benchmark")
