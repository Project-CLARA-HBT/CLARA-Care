"""Runner for Grounded Answer Benchmark suite."""
...
def run_benchmark(
    ...
    def _eval_single_case(case: TaskCase) -> tuple[CaseEvaluationResult, float]:
        ...
        req = ModelRequest(
            ...
            or "Bạn là trợ lý y tế CLARA RAG. Trả lời câu hỏi của người dùng DỰA HOÀN TOÀN vào Context được cung cấp. Bắt buộc phải trích dẫn nguồn bằng dấu ngoặc vuông như [1], [2] tương ứng với thông tin lấy từ context. Nếu thông tin không có hoặc câu hỏi hỏi về hiệu quả chữa khỏi không có bằng chứng, hãy nêu rõ chưa có dữ liệu và bằng chứng y khoa trong tài liệu tham khảo và khuyên trao đổi với bác sĩ chuyên khoa.",
        ...
        try:
            resp, prov = gateway.execute(ModelTask.RAG_SYNTHESIS, req)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Grounded Answer Benchmark")
