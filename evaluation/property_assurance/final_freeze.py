...
def _validate_target_hashes(*, repository_root: Path, methods: object) -> None:
    ...
    for method in METHOD_IDS:
        ...
        for target in targets:
            ...
            if not path.is_file() or _require_sha256(
                hashes[target], "govmut_final_freeze_target_hash_invalid"
            ) != sha256(path):
                raise FreezeError("govmut_final_freeze_target_hash_mismatch")


def _validate_review_artifact(
    *, manifest_path: Path, review: object, expected_ids: set[str]
) -> None:
    if (
    ...
    if _require_sha256(
        review.get("results_sha256"), "govmut_final_freeze_review_artifact_hash_invalid"
    ) != sha256(artifact):
    ...
    for disposition in dispositions:
        if (
            ...
            or not all(
                value in _REVIEW_DISPOSITIONS
                for value in disposition["model_dispositions"].values()
            )
        ):
...
def validate_final_freeze(
    *, manifest_path: Path, repository_root: Path, catalog_path: Path, statistics_plan_path: Path
    ...
    if _require_sha256(
        manifest["catalog_sha256"], "govmut_final_freeze_catalog_hash_invalid"
    ) != sha256(catalog_path):
        raise FreezeError("govmut_final_freeze_catalog_hash_mismatch")
    if _require_sha256(
        manifest["statistics_plan_sha256"], "govmut_final_freeze_statistics_hash_invalid"
    ) != sha256(statistics_plan_path):
    ...
    if (
        ...
        or not isinstance(hypothesis.get("version"), str)
        or not hypothesis["version"].strip()
    ):
    ...
    if (
        ...
        or required_limits - set(limits)
        or not all(isinstance(limits[key], int) and limits[key] > 0 for key in required_limits)
    ):
