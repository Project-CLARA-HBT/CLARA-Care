...
def _manifest(root: Path, catalog: Path, statistics: Path) -> dict[str, object]:
    ...
    return {
        ...
        "methods": {
            method: {"targets": targets, "target_sha256": {targets[0]: _hash(target)}}
            for method in (
                "M0_regression",
        ...
        "limits": {
            ...
            "hypothesis_stateful_step_count": 25,
        },
...
def _files(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    catalog.write_text(
        json.dumps({"candidates": [{"id": "M01-A"}, {"id": "M02-A"}]}), encoding="utf-8"
    )
                ...
                "dispositions": [
                    {
                        ...
                        "model_dispositions": {
                            "gemini-3.6-flash-high": "included",
                            "claude-sonnet-4-6": "included",
                        },
                    },
                    {
                        "mutant_id": "M02-A",
                        "disposition": "excluded_equivalent",
                        "model_dispositions": {
                            "gemini-3.6-flash-high": "excluded_equivalent",
                            "claude-sonnet-4-6": "excluded_equivalent",
                        },
                    },
                ],
            }
...
def test_final_freeze_requires_one_valid_disposition_per_catalog_mutant(
    ...
    with pytest.raises(
        FreezeError, match="govmut_final_freeze_review_disposition_coverage_invalid"
    ):
        validate_final_freeze(
            manifest_path=manifest,
            repository_root=root,
