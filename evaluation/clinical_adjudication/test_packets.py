...
def _input(path: Path) -> None:
    path.write_text(
        json.dumps(
            {
                "case_id": "secure-case-1",
                "review_payload": {"timeline": ["deidentified event"]},
...
def test_blinded_export_import_and_disagreement_packets(tmp_path: Path) -> None:
    ...
    manifest = export_packets(
        ...
        blinding_salt="a-long-controlled-test-salt",
    )
    ...
    annotation_manifest.write_text(
        json.dumps(
            {
                "status": "frozen",
                "annotation_guide_sha256": sha256(guide),
    ...
    with decisions.open("w", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(
            stream, fieldnames=["packet_id", "adjudicator_id", "field", "final_label", "rationale"]
        )
        writer.writeheader()
        writer.writerow(
            {
                ...
                "field": "current_state",
                "final_label": "active",
                "rationale": "documented human rationale",
            }
        )
    final = tmp_path / "final.csv"
    adjudicated = import_adjudications(
        adjudications_path=decisions,
        disagreement_path=adjudication,
        packet_dir=packets,
...
def test_export_rejects_system_identity_in_reviewer_payload(tmp_path: Path) -> None:
    ...
    source.write_text(
        json.dumps({"case_id": "case", "review_payload": {"model": "forbidden"}, "fields": ["x"]})
        + "\n",
        encoding="utf-8",
    )
    guide.write_text("rubric", encoding="utf-8")
...
def test_adjudication_rejects_annotation_manifest_from_another_packet_set(tmp_path: Path) -> None:
    ...
    annotation_manifest.write_text(
        json.dumps(
            {
                "status": "frozen",
                "annotation_guide_sha256": "wrong-packet-guide",
                "annotator_ids": ["reviewer-a", "reviewer-b"],
                "adjudicator_id": "adjudicator-c",
                "oracle_sha256": "oracle-sha",
                "blinding": "system-blinded",
