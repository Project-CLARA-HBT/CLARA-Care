...
def _has_forbidden_key(value: object) -> bool:
    if isinstance(value, dict):
        return any(
            str(key).lower() in _FORBIDDEN_PAYLOAD_KEYS or _has_forbidden_key(item)
...
def export_packets(
    *,
    ...
    if (
        ...
        or any(not item.strip() for item in reviewer_ids)
    ):
    ...
    for row in _jsonl(input_path):
        if (
            _INPUT_FIELDS - row.keys()
            or not isinstance(row["review_payload"], dict)
    ...
    *,
    ...
    try:
        ...
        mapping_reader = csv.DictReader(
            (packet_dir / "controlled_packet_map.csv").open(encoding="utf-8", newline="")
        )
    ...
    if manifest.get("schema_version") != SCHEMA_VERSION or manifest.get(
        "controlled_packet_map_sha256"
    ) != sha256(packet_dir / "controlled_packet_map.csv"):
...
def disagreement_packets(
    *, labels_path: Path, packet_dir: Path, output_path: Path
    ...
    result = import_labels(
        ...
        output_path=output_path.with_suffix(".normalized.csv"),
    )
    rows = list(
        csv.DictReader(
            output_path.with_suffix(".normalized.csv").open(encoding="utf-8", newline="")
        )
    )
    map_rows = list(
        csv.DictReader(
    ...
    reviewers = set(
        json.loads((packet_dir / "packet_manifest.json").read_text(encoding="utf-8"))[
            "reviewer_ids"
        ]
    )
    disagreements = [
    ...
    return {
        **result,
        ...
        "adjudication_packets_sha256": sha256(output_path),
    }
...
def import_adjudications(
    *,
    ...
    if disagreements.get("schema_version") != SCHEMA_VERSION or not isinstance(
        disagreements.get("disagreements"), list
    ):
        raise FreezeError("adjudication_packet_invalid")
    if packet_manifest.get("schema_version") != SCHEMA_VERSION or packet_manifest.get(
        "controlled_packet_map_sha256"
    ) != sha256(packet_dir / "controlled_packet_map.csv"):
        raise FreezeError("packet_map_manifest_mismatch")
    if set(annotation["annotator_ids"]) != set(
        packet_manifest.get("reviewer_ids", [])
    ...
    expected = {
        (str(row["packet_id"]), str(row["field"])) for row in disagreements["disagreements"]
    }
    ...
    for row in reader:
        packet_id, reviewer, field, label, rationale = (
            str(row[name]).strip() for name in _ADJUDICATION_FIELDS
        )
        key = (packet_id, field)
        if (
            reviewer != adjudicator
            or key not in expected
            or packet_id not in mapping
            or not label
            or not rationale
            or key in seen
        ):
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    _write_csv(
        output_path, ("case_id", "adjudicator_id", "field", "final_label", "rationale"), resolved
    )
    result = {
        "schema_version": SCHEMA_VERSION,
    ...
    try:
        if args.command == "export":
            result = export_packets(
                ...
                reviewer_ids=tuple(args.reviewer),
                blinding_salt=args.blinding_salt,
            )
        elif args.command == "import-labels":
            result = import_labels(
                labels_path=args.labels, packet_dir=args.packet_dir, output_path=args.output
            )
        elif args.command == "disagreements":
            result = disagreement_packets(
                labels_path=args.labels, packet_dir=args.packet_dir, output_path=args.output
            )
        else:
            result = import_adjudications(
                adjudications_path=args.adjudications,
                disagreement_path=args.disagreements,
