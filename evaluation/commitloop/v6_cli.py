"""Fail-closed CLI for a frozen GLHS-Bench v6 non-final partition."""
...
def main() -> int:
    ...
    if not isinstance(split_subjects, int) or not isinstance(request_count, int):
        raise TypeError("v6_cli_split_contract_missing")
    limits = RunLimits(
    ...
    if not base_url or not api_key:
        raise ValueError("v6_cli_router_configuration_missing")
    clients = {
    ...
    result = run_v6_development_partition(
        rows=rows,
        split=args.split,
