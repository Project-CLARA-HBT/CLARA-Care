"""Generate the fresh source-disjoint v7 confirmatory GLHS-Bench cohort.

The template families, seeds, schema version and subject tokens are all
independent of the sealed v5/v6 cohorts.  Split membership is allocated before
any runner is invoked: development and validation are available for iteration,
while ``sealed_test`` is withheld for one final candidate execution.  Prior-
cohort exclusion against the sealed v5/v6 evidence is enforced by the freeze
via the prior-cohort exclusion registry (see ``v7_freeze.py``).
"""
...
COHORT_NAME = "glhs_bench_v7_confirmatory_768"
MASTER_SEED = 2026081907
VALID_CUTOFF = datetime(2027, 6, 1, tzinfo=UTC)
KNOWN_CUTOFF = datetime(2027, 6, 1, tzinfo=UTC)
SPLIT_COUNTS = {"development": 24, "validation": 24, "sealed_test": 48}
STRATA = (
...
def _events(stratum: str) -> tuple[str, list[dict[str, str]]]:
    ...
    raise ValueError("unknown_v7_stratum")
...
def _bundle(
    *, stratum: str, split: str, index: int, seed: int, schema_version: str
    ...
    subject_id, code = f"v7-{opaque[:20]}", f"v7-{opaque[20:36]}"
        ...
        for split_index, (split, count) in enumerate(SPLIT_COUNTS.items()):
            for index in range(count):
                ...
                if gold.get("status") != "SCORABLE":
                    raise ValueError("v7_cohort_gold_not_scorable")
                bundle_hash = _digest(bundle)
                if token in tokens or bundle_hash in bundles:
                    raise ValueError("v7_cohort_duplicate_subject_or_bundle")
                tokens.add(token)
...
def bundles_for_split(
    ...
    if split not in SPLIT_COUNTS:
        raise ValueError("v7_split_invalid")
    selected = [row for row in rows if row["split"] == split]
    if len(selected) != len(STRATA) * SPLIT_COUNTS[split]:
        raise ValueError("v7_split_inventory_invalid")
    return (
