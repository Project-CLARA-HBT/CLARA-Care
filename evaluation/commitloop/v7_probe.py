"""Probe and record the exact two-model v7 router contract."""
...
def probe_v7_models(
    *,
    ...
    if set(clients) != set(CONFIRMATORY_MODELS):
        raise ValueError("v7_probe_exact_models_required")
    results = []
    for model in CONFIRMATORY_MODELS:
        ...
        if parse_json_object_content(result.content) != {"status": "ok"}:
            raise ValueError("v7_probe_response_invalid")
        results.append(
            {
