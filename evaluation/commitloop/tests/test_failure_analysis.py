        ...
        "entry": [
            {"resource": {"resourceType": "Patient", "id": "analysis-subject"}},
            {
                "resource": {
                    ...
                    "id": "request-analysis",
                    "status": "active",
                    "subject": {"reference": "Patient/analysis-subject"},
                    "authoredOn": "2026-01-01T00:00:00Z",
                    "occurrencePeriod": {"end": "2026-01-20T00:00:00Z"},
                    "code": {"coding": [{"system": "http://loinc.org", "code": "analysis-code"}]},
                }
            },
            {
                "resource": {
...
def test_failure_analysis_is_aggregate_and_refuses_final_holdout(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="final_holdout_failure_analysis_forbidden"):
        analyze_development_run(
            run_dir=run, output_dir=tmp_path / "forbidden", analysis_split="sealed_test"
        )
