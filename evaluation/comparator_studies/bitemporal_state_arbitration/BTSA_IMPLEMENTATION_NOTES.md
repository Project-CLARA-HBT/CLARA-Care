...
A source-availability review on 2026-08-11 found that the paper states the
evaluation framework and Med-LongMem diabetes suite are released, but the
paper contains no repository URL, revision, license, prompt, or artifact hash,
and an exact-title/author search found no official runnable release. This is an
availability finding, not evidence that no release can ever exist; fidelity
must be reassessed if a source-owned artifact becomes available.
