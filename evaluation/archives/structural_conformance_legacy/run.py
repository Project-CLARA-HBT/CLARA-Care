...
def _case(index: int) -> Case:
    ...
    return Case(
        ...
        experiment=(
            "direct_conformance"
            if index <= 100
            else "ambiguity_escalation"
            if index > 340
            else "compositional_stress"
        ),
...
def _load_completed_model_arm(
    source: Path, cases: Iterable[Case]
        ...
        ledger.append(
            {
                ...
                "status": "completed"
                if all(row.get("status") == "completed" for row in group) and len(group) == 120
                ...
                "degraded_or_fallback_indicated": sum(
                    row.get("degraded") == "True" for row in group
                ),
            ...
            output.append(
                {
                    ...
                    "unauthorized_disclosure_numerator": sum(
                        row.unauthorized_disclosure for row in selected
                    ),
                    "gst_bypass_numerator": sum(row.gst_bypass for row in selected),
                    "safe_escalation_numerator": sum(
                        row.automation == "safe_escalation" for row in selected
                    ),
                    "incorrect_automation_numerator": sum(
                        row.automation == "incorrect_automation" for row in selected
                    ),
                }
            )
    return output
...
def _operational_metrics(rows: list[Outcome]) -> list[dict[str, object]]:
    ...
    conflicts = [
        row
        for row in full
        if row.scenario in {"conflict", "scribe_ambiguity", "temporal_ambiguity"}
    ]
    auth = [row for row in full if row.scenario in {"family_isolation", "consent_revocation"}]
    return [
        {
            "metric": "provenance_coverage",
            "numerator": sum(row.provenance_complete for row in full),
            "denominator": len(full),
        },
        {
            "metric": "reconstruction_fidelity",
            "numerator": sum(row.reconstruction_correct for row in full),
            "denominator": len(full),
        },
        {
            "metric": "conflict_safe_escalation",
            "numerator": sum(row.automation == "safe_escalation" for row in conflicts),
            "denominator": len(conflicts),
        },
        {
            "metric": "policy_prohibited_disclosure",
            "numerator": sum(row.unauthorized_disclosure for row in full),
            "denominator": len(full),
        },
        {
            "metric": "ai_direct_write_bypass",
            "numerator": sum(row.gst_bypass for row in direct_writes),
            "denominator": len(direct_writes),
        },
        {
            "metric": "stale_transition_bypass",
            "numerator": sum(
                row.gst_bypass for row in full if row.scenario == "stale_state_version"
            ),
            "denominator": sum(row.scenario == "stale_state_version" for row in full),
        },
        {
            "metric": "revocation_honored",
            "numerator": sum(row.revocation_honored for row in auth),
            "denominator": len(auth),
        },
        {
            "metric": "automation_correct_state",
            "numerator": sum(row.automation == "correct_state" for row in full),
            "denominator": len(full),
        },
        {
            "metric": "human_escalation",
            "numerator": sum(row.automation == "safe_escalation" for row in full),
            "denominator": len(full),
        },
    ]
...
def _comparisons(rows: list[Outcome], seed: int) -> dict[str, dict[str, object]]:
    ...
    for baseline in (
        ...
        if not subjects or set(subjects) != set(baseline_by_subject):
            continue
        test = mcnemar_exact(
            [all(full_by_subject[s]) for s in subjects],
            [all(baseline_by_subject[s]) for s in subjects],
        )
        ...
        by_subject = {
            subject: [
                ...
                for left, right in zip(
                    full_grouped[subject], baseline_grouped[subject], strict=True
                )
            ]
        ...
        boots = [
            sum(rng.choices(mean_values, k=len(mean_values))) / len(mean_values)
        ...
        comparisons[key] = {
            ...
            "patient_bootstrap_risk_difference": {
    ...
    for key, adjusted in holm_adjust(raw_p).items():
        comparisons[key]["state_correct_mcnemar_exact_subject_summary"]["holm_adjusted_p_value"] = (
            adjusted
        )
    return comparisons
    ...
    return {
        ...
        "protocol": {
            ...
            "model_backed_execution": {
                ...
                "status": "completed"
                if model_arm_summary is not None
                else "not_run_no_frozen_model_provider_config",
                "artifact": "model_arm_summary.json" if model_arm_summary is not None else None,
                "aggregate": model_arm_summary.get("aggregate")
                if model_arm_summary is not None
                else None,
            },
            "limitations": [
        ...
        "external_raw": {
            **external_raw,
            "stratified_metrics": external_stratified,
...
def _write_frozen_inputs(output: Path, cases: list[dict[str, object]]) -> dict[str, str]:
    ...
    policy = {
        ...
        "rules": {
            "late_evidence": "retain_valid_time",
            "equal_authority": "preserve_conflict",
            "stale_transition": "reject_or_reconcile",
            "ai_write": "proposal_only",
        },
    }
    relevance = {
        "schema_version": "glhs-q2-task-relevance-v1",
        "builder_uses_thss": False,
        "critical_fact_count": 3,
        "nonessential_authorized_fact_count": 7,
        "profiles": list(THSS_PROFILES),
    }
    oracle = {
        "schema_version": "glhs-q2-oracle-v1",
        "builder_uses_production_gst_or_thss": False,
        "cases": cases,
    }
    holdout = {
        "schema_version": "glhs-q2-holdout-v1",
        "holdout_inspected_during_tuning": False,
        "case_ids": [row["case_id"] for row in cases if row["partition"] == "sealed_holdout"],
        "template_seed": 20260808,
    }
    ...
    mechanism = {
        ...
        "entries": [
            {
                "comparator": "openEHR RM",
                "citation": "[12] openEHR Reference Model",
                "inclusion_criterion": "published specification explicitly describes EHR version/access/content model",
                "cells": {
                    "valid_knowledge_time": "Partial",
                    "provenance_linked_state": "Yes",
                    "historical_replay": "Yes",
                    "governed_mutation_boundary": "Partial",
                    "purpose_bounded_projection": "NR",
                    "revocation_aware_ai_context": "Partial",
                },
                "glhs_distinction": "GST and THSS share state/policy-version constraints.",
            },
            {
                "comparator": "FHIR Provenance + Consent",
                "citation": "[10] FHIR Provenance; [11] FHIR Consent",
                "inclusion_criterion": "published FHIR R4 resources explicitly describe provenance and consent representation",
                "cells": {
                    "valid_knowledge_time": "Partial",
                    "provenance_linked_state": "Partial",
                    "explicit_unresolved_conflict": "NR",
                    "historical_replay": "Partial",
                    "governed_mutation_boundary": "NR",
                    "purpose_bounded_projection": "Partial",
                    "revocation_aware_ai_context": "Partial",
                },
                "glhs_distinction": "FHIR resources represent facts; they do not themselves define the coupled GST/THSS execution contract.",
            },
            {
                "comparator": "TRACE",
                "citation": "[6] TRACE",
                "inclusion_criterion": "cited system explicitly reports structured temporal patient state",
                "cells": {
                    "valid_knowledge_time": "Yes/structured temporal state",
                    "provenance_linked_state": "NR",
                    "explicit_unresolved_conflict": "NR",
                    "historical_replay": "NR",
                    "governed_mutation_boundary": "Partial",
                    "purpose_bounded_projection": "NR",
                    "revocation_aware_ai_context": "NR",
                },
                "glhs_distinction": "No cited shared version-coupled mutation and disclosure boundary.",
            },
            {
                "comparator": "Dual-Stream Reconciliation",
                "citation": "[15] Pugh et al.",
                "inclusion_criterion": "cited architecture explicitly reports discrepancy reconciliation",
                "cells": {
                    "valid_knowledge_time": "Partial",
                    "provenance_linked_state": "Partial",
                    "explicit_unresolved_conflict": "Yes",
                    "historical_replay": "NR",
                    "governed_mutation_boundary": "Partial",
                    "purpose_bounded_projection": "NR",
                    "revocation_aware_ai_context": "NR",
                },
                "glhs_distinction": "Not reported as a general version-coupled lifecycle across heterogeneous sources.",
            },
            {
                "comparator": "Baichuan-M4",
                "citation": "[7] Baichuan-M4",
                "inclusion_criterion": "cited system explicitly reports long-term memory and action constraints",
                "cells": {
                    "valid_knowledge_time": "Partial",
                    "provenance_linked_state": "Partial",
                    "explicit_unresolved_conflict": "NR",
                    "historical_replay": "NR",
                    "governed_mutation_boundary": "Partial/action constraints",
                    "purpose_bounded_projection": "NR",
                    "revocation_aware_ai_context": "NR",
                },
                "glhs_distinction": "No cited base-state-version precondition and task/purpose-bounded snapshot.",
            },
            {
                "comparator": "VISTA Architect",
                "citation": "[16] VISTA Architect",
                "inclusion_criterion": "cited system explicitly reports provenance-linked persistent state and temporal synthesis",
                "cells": {
                    "valid_knowledge_time": "Yes/temporal synthesis",
                    "provenance_linked_state": "Yes",
                    "explicit_unresolved_conflict": "Partial/deduplication",
                    "historical_replay": "Partial/source graph",
                    "governed_mutation_boundary": "NR",
                    "purpose_bounded_projection": "NR",
                    "revocation_aware_ai_context": "NR",
                },
                "glhs_distinction": "No cited GST-like gate coupled to revocation-aware THSS disclosure.",
            },
        ],
    ...
    payloads = {
        "policy.json": policy,
        "task_relevance_manifest.json": relevance,
        "oracle_manifest.json": oracle,
        "holdout_manifest.json": holdout,
        "mechanism_evidence.json": mechanism,
    }
    hashes: dict[str, str] = {}
...
def write(
    ...
    *,
    ...
    _write_csv(
        output / "conformance.csv",
        [row for row in result["outcomes"] if row["experiment"] == "direct_conformance"],
    )
    _write_csv(
        output / "ablation.csv",
        [
            {"comparison": name, **value["patient_bootstrap_risk_difference"]}
    ...
    for name, value in result["comparisons"].items():
        comparison_rows.append(
            {
                "comparison": name,
                **value["state_correct_mcnemar_exact_subject_summary"],
    ...
    if external.get("status") == "evaluated_deidentified_structural_perturbations":
        ...
        _write_csv(
            output / "external_baseline_comparison.csv",
            [
                {
                    "comparison": name,
                    **value["state_correct_mcnemar_exact_subject_summary"],
                ...
                for name, value in external_raw["comparisons"].items()
            ],
        )
    else:
    ...
    manifest = {
        "schema_version": "glhs-q2-evidence-manifest-v1",
...
def main() -> None:
    ...
    completed_model_arm = (
        _load_completed_model_arm(
            args.model_arm_source.resolve(), [_case(index) for index in range(1, args.cases + 1)]
        )
    ...
    write(
        ...
        args.output,
        frozen_input_sha256=frozen_input_sha256,
        model_arm_source=args.model_arm_source.resolve()
        if args.model_arm_source is not None
        else None,
    )
