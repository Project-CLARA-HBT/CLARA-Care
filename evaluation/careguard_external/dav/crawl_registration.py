...
def crawl(root: Path, retrieval_date: str) -> int:
    ...
    if inventory.exists():
        completed = {json.loads(line).get("url") for line in inventory.read_text(encoding="utf-8").splitlines()}
    failed_path = store.manifests / "failed_or_unavailable.jsonl"
    if failed_path.exists():
        completed |= {json.loads(line).get("url") for line in failed_path.read_text(encoding="utf-8").splitlines()}
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for row, res in executor.map(_fetch_one, to_fetch):
            if isinstance(res, Exception):
                store.append("failed_or_unavailable.jsonl", {"url": row["url"], "retrieved_at_utc": now(), "reason": str(res)})
            else:
                status, content_type, payload, final_url = res
                source = store.retain(bucket=row["document_type"], url=final_url, payload=payload, content_type=content_type, status=status)
                source.update({"source_url": final_url, "title": row["title"], "document_type": row["document_type"], "decision_number": decision_number(row["title"]), "discovered_from": row["discovered_from"]})
