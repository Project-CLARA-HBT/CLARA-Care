...
EVENT_TYPES = frozenset({"ISSUED", "WITHDRAWN", "RENEWED", "CONTINUED_VALIDITY", "AMENDED", "CORRECTED"})
...
def reconstruct(root: Path, as_of: str, events: list[dict], unresolved: list[dict]) -> list[dict]:
    ...
    unresolved_numbers = {row["registration_number"] for row in unresolved}; groups: dict[str, list[dict]] = {}
    for event in events:
    ...
    for number, chain in sorted(groups.items()):
        ...
        status = "STATUS_UNRESOLVED" if number in unresolved_numbers else "WITHDRAWN" if "WITHDRAWN" in kinds else "HISTORICAL"
        rows.append({"registration_number": number, "status_as_of": as_of, "status": status, "evidence_document_ids": [event["evidence_document_id"] for event in chain]})
    write_parquet(rows, root / "normalized" / f"status_as_of_{as_of}.parquet"); return rows
