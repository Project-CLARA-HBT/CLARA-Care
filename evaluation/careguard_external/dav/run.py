...
def main() -> int:
    ...
    discover(args.root, args.date)
    crawl_publications(args.root, args.date)
    crawl_registration(args.root, args.date)
    download(args.root, args.date)
    parse(args.root, args.date)
    products, events = normalize(args.root, args.date)
    sources, unresolved = reconcile(args.root, events)
    statuses = reconstruct(args.root, args.date, events, unresolved)
    validation = validate(args.root, args.date)
    manifests = args.root / "manifests"
    inventory = manifests / "source_inventory.jsonl"
    digest = hashlib.sha256(inventory.read_bytes()).hexdigest()
    (manifests / "files.sha256").write_text(
        "".join(
            f"{row['sha256']}  {row['raw_path']}\n"
            for row in [
    ...
    manifest = {
        "schema_version": "dav.acquisition.v1",
        "scope": "DAV public medication-product records only",
        "crawl_started_at_utc": started.isoformat(),
        "crawl_ended_at_utc": datetime.now(UTC).isoformat(),
