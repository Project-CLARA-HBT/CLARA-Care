...
def parse(root: Path, retrieval_date: str) -> list[dict]:
    ...
    for row in rows:
        ...
        if "attachments" not in path.parts:
            continue
        result = {
            "raw_artifact_sha256": row["sha256"],
            "source_url": row["url"],
        ...
        try:
            ...
            if path.suffix.lower() == ".pdf":
                ...
                result["text"] = completed.stdout.decode("utf-8", "replace")
                result["parse_status"] = "PARSED"
            elif path.suffix.lower() == ".csv":
                result["text"] = path.read_text(encoding="utf-8-sig")
                result["parse_status"] = "PARSED"
            elif path.suffix.lower() == ".docx":
                result["text"] = parse_docx(path)
                result["parse_status"] = "PARSED"
            elif path.suffix.lower() == ".xlsx":
                result["text"] = parse_xlsx(path)
                result["parse_status"] = "PARSED"
            else:
                result["parse_error"] = f"no_parser_for:{path.suffix.lower() or 'no_extension'}"
            if result["parse_status"] == "PARSED" and not result["text"].strip():
                result["parse_status"] = "PARSED_EMPTY"
                result["parse_error"] = "no_extractable_text"
    ...
    output = store.manifests / "attachment_parse.jsonl"
    output.write_text(
...
if __name__ == "__main__":
    ...
    print(len(parse(args.root, args.date)))
