...
def validate(root: Path, retrieval_date: str) -> dict:
    ...
    url_bytes: dict[str, str] = {}
    errors = []
    for row in rows:
        path = Path(row["raw_path"])
        if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest() != row["sha256"]:
            errors.append(f"artifact_hash_mismatch:{row['url']}")
        prior = url_bytes.setdefault(row["url"], row["sha256"])
        if prior != row["sha256"]:
            errors.append(f"url_maps_to_multiple_bytes:{row['url']}")
    ...
    if parse_manifest.exists():
        ...
        for parsed in (
            json.loads(line) for line in parse_manifest.read_text(encoding="utf-8").splitlines()
        ):
            ...
            if status == "PARSED" and (
                not isinstance(parsed.get("text"), str) or not parsed["text"].strip()
            ):
                errors.append("parsed_record_missing_text")
            if status == "PARSED_EMPTY" and (
                not isinstance(parsed.get("text"), str) or parsed["text"].strip()
            ):
