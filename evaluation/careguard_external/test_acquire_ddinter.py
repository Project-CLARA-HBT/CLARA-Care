...
def test_inventory_is_complete_and_manifest_is_source_valid(tmp_path: Path) -> None:
    ...
    for index, name in enumerate(DDINTER_FILES):
        (archive / name).write_text(
            f"drug_a,drug_b,severity\ndrug-{index},other-{index},major\n",
            encoding="utf-8",
        )
    ...
    assert (
        inventory[0]["source_record_hash"]
        == hashlib.sha256(
    ...
    assert (
        verify_archive(manifest_path=manifest_path)["verification_status"]
        == "archive_matches_manifest"
    )

    (archive / DDINTER_FILES[0]).write_text(
