...
### GLHS Journal
- Historical v1 run `GLHS-POSTGRES-TOCTOU-FINAL-20260817-01` (5 schedules) is retained unchanged; its one concurrent commit remains `INDETERMINATE`.
- Canonical v2 run `GLHS-POSTGRES-TOCTOU-FINAL-V2-20260817-01` (12 schedules, source `67e528b6`) is byte-verified, with 10 rejected, 2 committed, 0 forbidden commits, and 2 expected/observed classification mismatches.
- V2 is **not claim-eligible** under its frozen result rule until `TOCTOU-V2-05` and `TOCTOU-V2-09` are reconciled and resealed. No universal concurrency or provenance-retention guarantee is claimed.
- Formal assurance: 21,361 states / 90,432 transitions, 0 invariant violations (bounded, not universal proof).
- Gate boxes: persisted writers PASS; ordering evidence is partial; mismatch reconciliation, false-stale burden, and concurrency scaling remain open. Not a complete top-tier concurrency study yet.
...
CareGuard-VN: RESULT-INCOMPLETE (PAUSED_BY_OPERATOR).
