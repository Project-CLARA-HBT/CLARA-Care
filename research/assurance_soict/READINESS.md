...
The existing Hypothesis properties and hand-authored gateway regressions are implementation substrate only. An in-process state-machine now checks idempotent replay, stale-write rejection, and disclosure-followed-by-policy-mutation rejection against a small reference model; it is development evidence, not a mutation result or API-bound result. The 15-row fault-family inventory is not an executable mutant corpus; the target is at least 45 real, noncosmetic, semantically distinct executable mutants if the code supports it. The locked contribution is a health-AI governance fault taxonomy, executable mutant corpus, and comparative adequacy evidence—not novel PBT, state-machine, or mutation testing.
...
Remaining blockers: a production-facing state-machine adapter, test-only temporary mutant runner, frozen seeds/limits/statistics plan, M0–M3 executions, exact counterexample traces, a sealed assurance run, and a supported manuscript source.
...
and rejects cosmetic or non-unique anchors. Forty-five family-seed variants have
verified unique anchors in `mutation_site_candidates.json`, including direct
gateway and commitment-gateway enforcement sites. None of the newly added
variants has been executed
against the complete M0–M3 matrix and three families remain unanchored. This is
not yet an executable corpus, a mutation-score denominator, or an adequacy comparison.
