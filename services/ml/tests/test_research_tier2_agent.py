...
def test_build_retrieval_cache_summary_counts_hits_and_misses():
    cache_summary = tier2._build_retrieval_cache_summary(
        index_summary={
            "rerank": {"rerank_cache_hit": True, "rerank_cache_age_ms": 16.5},
        },
        deep_pass_summaries=[
            {"index_summary": {"rerank_cache_hit": False}},
            {
                "index_summary": {
                    "rerank": {
                        "rerank_cache_hit": True,
                        "rerank_cache_age_ms": 4.2,
                    }
                }
            },
        ],
    ...
    assert cache_summary["observed"] is True
    assert cache_summary["observation_count"] == 3
    assert cache_summary["hit_count"] == 2
    assert cache_summary["miss_count"] == 1
    assert cache_summary["hit_rate"] == pytest.approx(2 / 3, rel=1e-3)
    assert cache_summary["latest_cache_hit"] is True
    assert cache_summary["latest_cache_age_ms"] == pytest.approx(4.2, rel=1e-3)


def test_run_research_tier2_exposes_cache_summary_in_fast_mode(monkeypatch):
    def _fake_pipeline_run(self, query: str, **kwargs) -> RagResult:  # pragma: no cover - helper
        return RagResult(
            query=query,
            retrieved_ids=["cache-doc-1"],
            answer="Ná»™i dung táº¡m.",
            model_used="deepseek-v3.2",
            retrieved_context=[
                {
                    "id": "cache-doc-1",
                    "source": "internal",
                    "title": "Cache-aware row",
                    "text": "Cached retrieval row.",
                    "url": "",
                    "score": 0.91,
                }
            ],
            ...
            trace={
                "retrieval": {
                    "source_attempts": [{"provider": "internal_corpus"}],
                    "index_summary": {
                        "selected_count": 1,
                        "rerank": {
                            "rerank_cache_hit": True,
    ...
    assert cache_summary.get("observed") is True
    assert cache_summary.get("hit_count") == 1
    assert cache_summary.get("miss_count") == 0
    assert cache_summary.get("latest_cache_hit") is True

    telemetry = result.get("telemetry", {})
    index_summary = telemetry.get("index_summary", {})
    assert index_summary.get("rerank_cache_hit") is True
    assert isinstance(index_summary.get("cache_summary"), dict)
...
def test_apply_keyword_filter_to_query_plan_honors_source_language_profile():
    base_query_plan = {
        "original_query": "Warfarin and ibuprofen interaction",
        "canonical_query": "warfarin interaction with ibuprofen bleeding risk",
        "language_hint": "mixed",
        "source_queries": {
            "internal": ["warfarin ibuprofen"],
            "scientific": ["warfarin interaction clinical evidence"],
            "web": ["warfarin interaction guideline"],
        },
    }
    ...
    report = tier2._apply_keyword_filter_to_query_plan(
        topic="Warfarin and ibuprofen interaction",
        query_plan=base_query_plan,
        planner_keywords=["warfarin", "interaction", "bleeding", "tuong", "tac"],
        source_mode="davidrug",
    ...
    target_language_by_source = report.get("target_language_by_source", {})
    assert target_language_by_source.get("scientific") == "vi"
    assert target_language_by_source.get("web") == "en"
    assert target_language_by_source.get("internal") == "vi"

    normalized_profile = report.get("source_language_profile", {})
    assert normalized_profile == {"scientific": "vi", "web": "en"}
    query_plan = report.get("query_plan", {})
    assert query_plan.get("source_language_profile") == {"scientific": "vi", "web": "en"}
