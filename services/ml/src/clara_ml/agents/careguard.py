...
def _load_drugbank_ddi_rules() -> tuple[list[InteractionRule], str]:
    """Load DrugBank DDI shards (additive layer).
    ...
    mirroring ``_load_local_ddi_rules``. This loader is only ever invoked when the
    ...
    if _DRUGBANK_DDI_CACHE_MTIME_NS == mtime_ns and _DRUGBANK_DDI_CACHE_RULES:
        return _DRUGBANK_DDI_CACHE_RULES, _DRUGBANK_DDI_CACHE_VERSION

    try:
        manifest = json.loads(_DRUGBANK_MANIFEST_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return _DRUGBANK_DDI_CACHE_RULES, _DRUGBANK_DDI_CACHE_VERSION

    version = str(manifest.get("version") or "drugbank").strip() or "drugbank"
