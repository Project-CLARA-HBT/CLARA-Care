"""Shared fixtures for the regulatory-compliance property suite.

These compose with the repository-root ``tests/conftest.py`` (DB schema +
bootstrap admin + per-test row reset). They give every compliance property test
a consistent way to flip ``COMPLIANCE_*`` flags and to read a fresh, flags-off
``Settings`` baseline.
"""
...
def flags_off_settings() -> Settings:
    """A freshly constructed ``Settings`` with no compliance env overrides.

    Used by the flags-off baseline assertion (Property P6 at the config layer).
    """
...
def set_flags(monkeypatch: pytest.MonkeyPatch):
    ...
    Example::
        ...
        def test_x(set_flags):
            set_flags(compliance_dsar_enabled=True)
            ...
    """

    def _apply(**flags: bool) -> None:
        set_compliance_flags(monkeypatch, **flags)
