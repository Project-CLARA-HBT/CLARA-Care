"""Shared fixtures for the CLARA Council upgrade suite.

These compose with the repository-root ``tests/conftest.py`` (DB schema +
bootstrap admin + per-test row reset). They give every Council-upgrade property
test a consistent way to flip the new ``COUNCIL_*`` flags and to read a fresh,
flags-off ``Settings`` baseline.
"""
...
def flags_off_settings() -> Settings:
    """A freshly constructed ``Settings`` with no Council-upgrade env overrides.

    Used by the flags-off baseline assertion (design Property P8 at the config
    layer; Requirements 9.1, 9.2).
    """
...
def set_flags(monkeypatch: pytest.MonkeyPatch):
    ...
    def _apply(**flags: bool) -> None:
        set_council_flags(monkeypatch, **flags)
