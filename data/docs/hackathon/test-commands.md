...
## 0.2. Chạy canonical demo cases A/B/C (online/offline/legal)
Script này chạy đúng 3 case pitching:
1. Case A Online (`external_ddi_enabled=true`)
2. Case B Offline fallback (`external_ddi_enabled=false`)
3. Case C Legal trap (chatbot phải từ chối kê đơn/chẩn đoán/liều)

```bash
bash scripts/demo/run_round2_demo_cases.sh \
  --run-id round2-demo-cases \
  --api-base-url http://127.0.0.1:8000 \
  --email admin@example.com \
  --password 'Clara#Admin2026!' \
  --doctor-email admin@example.com \
...
Output:
- `artifacts/round2/<run_id>/demo-cases/case-a-online.json`
- `artifacts/round2/<run_id>/demo-cases/case-b-offline-fallback.json`
- `artifacts/round2/<run_id>/demo-cases/case-c-legal-trap.json`
- `artifacts/round2/<run_id>/demo-cases/demo-cases-summary.json`
- `artifacts/round2/<run_id>/demo-cases/demo-cases-summary.md`

## 0.3. Rà legal text + attribution cuối (artifact có thể nộp)

```bash
python3 scripts/demo/run_legal_attribution_review.py \
  --run-id round2-final-canonical-<timestamp> \
  --api-base-url https://clara.thiennn.icu \
  --email admin@example.com \
  --password 'Clara#Admin2026!' \
  --timeout-seconds 25
```
...
Output:
- `artifacts/round2/<run_id>/legal-attribution-review/legal-attribution-review.json`
- `artifacts/round2/<run_id>/legal-attribution-review/legal-attribution-review.md`
